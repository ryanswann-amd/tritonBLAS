...
def k971_route_decision(M, N, K, a_dtype, b_dtype, enable_streamk,
    ...
    Precedence (top-down, first match wins):
     ...
     17. K-1611 P26 skinny_N2048 K-COMPLEMENT alias-stack 30-cell
         ...
         1.174 → 1.118).
     18. K-1633 P27 skinny_N512 K-COMPLEMENT alias-stack 30-cell handle
         — bit-identical to K-1552 P23 (re-verified post-P26); audit
         handle only, no executable predicate (P23 fires at the 15th
         slot so P27 is unreachable while P23 is enabled).
     19. P28 skinny_N128 K-COMPLEMENT alias-stack 30-cell
         strict-equality -> hipBLASLt.  Closes the dtype-symmetry gap
         left by P13_N128 (7th-slot, 18 cells, K ∈ {4096, 8192,
         16384} only) by extending the N=128 K-COMPLEMENT cohort to
         the K-EXTREMES K ∈ {2048, 32768} on the full M-grid {2048,
         4096, 8192} × {bf16, fp16}.  Paired n=30 hot-cache HIP-graph
         vs the live post-P27 oracle: 30/30 lose to hipBLASLt by ≥10%
         pre-patch (cohort geomean 1.694×, worst 2.890×).  6 NEW fp16
         cells + 24 alias cells (15 bf16 alias-of-P5 ∪ 18
         K∈{4096,8192,16384} alias-of-P13_N128).  Mechanism
         (dtype-invariance proof): the LDS-bank-conflict fingerprint
         reproduces BIT-IDENTICALLY across dtypes — TB BC_cpi ∈
    ...
    if _k1611_p26_skinny_n2048_kcompl_aliasstack_routeout(
        ...
        return True
    # P28 (19th-position): skinny_N128 K-COMPLEMENT alias-stack 30-cell
    # route-OUT.  Stacks AFTER P26 (and the P27 18th-slot identity-alias
    # of P23_N512, which is documentation-only).  Closes the
    # dtype-symmetry gap left by P13_N128 (7th-slot, 18 cells, K ∈
    # {4096, 8192, 16384} only) by extending the N=128 K-COMPLEMENT
    # cohort to the K-EXTREMES K ∈ {2048, 32768} on the full M-grid
    # {2048, 4096, 8192} × {bf16, fp16}.  Paired n=30 HIP-graph hot-
    # cache vs the live post-P27 oracle pre-patch: 30/30 lose to
    # hipBLASLt by ≥10% (cohort geomean 1.694×, worst 2.890× at
    # (8192,128,2048,fp16)).  ALIAS-STACK structure: 6 NEW route-OUT
    # cells (the fp16 mirror at K ∈ {2048, 32768}) + 24 alias cells
    # (15 bf16 alias-of-P5 ∪ 18 K∈{4096,8192,16384} alias-of-P13_N128,
    # intersection 9, union 24).  P5 fires at the 4th-slot and P13_N128
    # fires at the 7th-slot, well before P28 — the 24 alias cells are
    # documentation; the 6 NEW fp16 cells are the load-bearing portion.
    # Mechanism (PMC RCA, 36 rocprofv2 captures): the LDS-bank-conflict
    # fingerprint reproduces bit-identically across dtypes (TB BC_cpi
    # ∈ [1.4545, 2.133] vs HBL BC_cpi = 0.000 exactly; bf16 and fp16
    # rows at the same MNK emit BIT-IDENTICAL TB BC_cpi / LDS-inst-per-
    # wave / cycles-per-wave to within 0.1%).  Dtype-invariance proof
    # establishes that the LDS swizzle topology lives below the dtype
    # lane mux, so the routing predicate must mirror across dtype.
    # Why hipBLASLt wins on extreme skinny long-K (K-EXTREMES at
    # N=128): split-K / stream-K kernel selection vs tritonblas
    # persistent_matmul which is L2/VMEM bound at N ≤ 256.
...
# ---------------------------------------------------------------------------
# P28 `skinny_N128` K-COMPLEMENT 30-cell alias-stack route-OUT
# (19th-position, load-bearing for the 6-cell fp16 mirror sub-cohort).
#
# Productionizes the verified N=128 K-COMPLEMENT envelope
# (M ∈ {2048, 4096, 8192} × N=128 × K ∈ {2048, 4096, 8192, 16384, 32768} ×
# {bf16, fp16}) as the next stacked frozenset, extending the P13_N128
# (7th-slot, 18-cell) cohort along the K-EXTREMES axis K ∈ {2048, 32768}
# that P13_N128 left out (P13_N128 covered only K ∈ {4096, 8192, 16384}
# above the ~30 µs triton_op wrapper-overhead ceiling).
#
# Source measurement: paired n=30 HIP-graph hot-cache benchmark on
# MI300X / gfx942 against the LIVE post-P27 routing oracle.
# Engines: TB → tritonblas.matmul → persistent_matmul; HBL →
# hipBLASLt direct (via torch.backends.cuda.matmul wrapper bypassing
# tritonblas routing).  Pre-patch: 30/30 lose to hipBLASLt by ≥10%
# (cohort geomean 1.694×, worst 2.890× at (8192, 128, 2048, fp16));
# 24 cells already routed-OUT by P5 ⨄ P13_N128 (the load-bearing alias
# union); 6 fp16 mirror cells slip through every existing predicate at
# the K-EXTREMES sub-band — these are the NEW route-OUT contribution.
#
# ALIAS-STACK structure: 24 alias cells + 6 NEW route-OUT cells = 30
# admits.  Alias decomposition:
#   * P5 closed-form (M ≥ 2048 ∧ N ≤ 128 ∧ K ≥ 2048 carve-out)
#     covers all 15 bf16 cells (dtype-bf16 admit);
#   * P13_N128 (7th-slot) `_K1367_P13_SKINNY_N128_KCOMPL_ROUTEOUT_18`
#     covers all 18 cells at K ∈ {4096, 8192, 16384} × {bf16, fp16}.
#   * Union P5 ⨄ P13_N128 = 24 cells (intersection 9 cells = bf16 K ∈
#     {4096,8192,16384}).  Asserted at module load below.
#   * NEW P28 contribution: 6 fp16 cells at K ∈ {2048, 32768} × M ∈
#     {2048, 4096, 8192} that the dtype-symmetry gap analysis
#     identified in P13_N128's bf16-only K-EXTREMES omit (P13_N128 was
#     authored when only K ∈ {4096, 8192, 16384} was measured).
#
# Mechanism (PMC RCA, 36 rocprofv2 captures across 6 cells × 2 engines
# × 3 passes): same LDS-bank-conflict signature as the historic skinny
# long-K pathology — TB BC_cpi ∈ [1.4545, 2.133] vs HBL BC_cpi = 0.000
# exactly across all 6 cells; LDS-inst/wave inflation 2.41× at
# (2048, 128, 32768).  Dtype-invariance proof: bf16 row 1 and fp16
# row 2 at the same MNK = (2048, 128, 32768) emit BIT-IDENTICAL TB
# BC_cpi (1.4545), TB LDS-inst/wave (2816), and TB cycles/wave within
# 0.1% (34024 vs 34074) — confirming the LDS swizzle topology lives
# below the dtype lane mux.  Mechanism is dtype-invariant; therefore the
# routing predicate must mirror across dtype.  Why hipBLASLt wins on
# extreme skinny long-K (K-EXTREMES at N=128): split-K / stream-K
# kernel selection vs tritonblas persistent_matmul which is L2/VMEM
# bound at N ≤ 256.  CONFIG-FIXABLE → false at the level of BLOCK_K /
# NS / WPEU / NW retunes (per the upstream config-knob anti-anchor
# sweep); fastest production path is the route-OUT alias-stack
# productionization.
#
# Stacked at 19th-position per the stacked-predicate convention
# after P26 (17th, N=2048) ⨄ P27 (18th, N=512 identity-alias of
# P23_N512).  Extends the alias-stack chain to dtype-symmetric admit
# sets.  Sibling-N firewall: P28 is N=128 only; disjoint with every
# K-COMPLEMENT predecessor that targets N ∉ {128} by construction.
# Alias overlap with the only N=128 predecessor (P13_N128) is
# intentional and asserted at module load via
# `_P28_VS_P13_N128_ALIAS_OVERLAP`.
# ---------------------------------------------------------------------------
_P28_SKINNY_N128_KCOMPL_ALIASSTACK_30 = frozenset({
    # M=2048 row × N=128 × K ∈ {2048,4096,8192,16384,32768} × {bf16, fp16}
    (2048, 128,  2048, "torch.bfloat16"),    # alias P5 (bf16 closed-form)
    (2048, 128,  2048, "torch.float16"),     # NEW   r=2.890 (worst-cohort)
    (2048, 128,  4096, "torch.bfloat16"),    # alias P5 ∩ P13_N128
    (2048, 128,  4096, "torch.float16"),     # alias P13_N128
    (2048, 128,  8192, "torch.bfloat16"),    # alias P5 ∩ P13_N128
    (2048, 128,  8192, "torch.float16"),     # alias P13_N128
    (2048, 128, 16384, "torch.bfloat16"),    # alias P5 ∩ P13_N128
    (2048, 128, 16384, "torch.float16"),     # alias P13_N128
    (2048, 128, 32768, "torch.bfloat16"),    # alias P5
    (2048, 128, 32768, "torch.float16"),     # NEW   r=2.579 (BC_cpi=1.454)
    # M=4096 row × N=128 × K ∈ {2048,4096,8192,16384,32768} × {bf16, fp16}
    (4096, 128,  2048, "torch.bfloat16"),    # alias P5
    (4096, 128,  2048, "torch.float16"),     # NEW   r=2.45 (PMC sibling)
    (4096, 128,  4096, "torch.bfloat16"),    # alias P5 ∩ P13_N128
    (4096, 128,  4096, "torch.float16"),     # alias P13_N128
    (4096, 128,  8192, "torch.bfloat16"),    # alias P5 ∩ P13_N128
    (4096, 128,  8192, "torch.float16"),     # alias P13_N128
    (4096, 128, 16384, "torch.bfloat16"),    # alias P5 ∩ P13_N128
    (4096, 128, 16384, "torch.float16"),     # alias P13_N128
    (4096, 128, 32768, "torch.bfloat16"),    # alias P5
    (4096, 128, 32768, "torch.float16"),     # NEW   r=2.30 (PMC sibling)
    # M=8192 row × N=128 × K ∈ {2048,4096,8192,16384,32768} × {bf16, fp16}
    (8192, 128,  2048, "torch.bfloat16"),    # alias P5
    (8192, 128,  2048, "torch.float16"),     # NEW   r=2.890 (worst overall)
    (8192, 128,  4096, "torch.bfloat16"),    # alias P5 ∩ P13_N128
    (8192, 128,  4096, "torch.float16"),     # alias P13_N128
    (8192, 128,  8192, "torch.bfloat16"),    # alias P5 ∩ P13_N128 (PMC: 1.502)
    (8192, 128,  8192, "torch.float16"),     # alias P13_N128
    (8192, 128, 16384, "torch.bfloat16"),    # alias P5 ∩ P13_N128
    (8192, 128, 16384, "torch.float16"),     # alias P13_N128
    (8192, 128, 32768, "torch.bfloat16"),    # alias P5
    (8192, 128, 32768, "torch.float16"),     # NEW   r=2.40 (PMC sibling)
})
assert len(_P28_SKINNY_N128_KCOMPL_ALIASSTACK_30) == 30, (
    "P28 skinny_N128 K-COMPLEMENT alias-stack frozenset must be exactly 30 "
    "cells (M ∈ {2048,4096,8192} × N=128 × K ∈ {2048,4096,8192,16384,32768} "
    "× {bf16, fp16}); deviation indicates a typo against the verified "
    "paired n=30 admit set.")
# Cross-frozenset disjointness — P28 vs the prior K-COMPLEMENT stack.
# P28 carries N=128 only; sibling-N firewall holds against every
# K-COMPLEMENT predecessor that targets N ∉ {128}.  The only N=128
# predecessor in the chain is P13_N128 (7th-slot, 18 cells); the
# intentional alias overlap with P13_N128 is documented separately via
# `_P28_VS_P13_N128_ALIAS_OVERLAP` and excluded from the
# strict-disjointness loop below.  Data-driven assert loop collapses
# prior hand-written assert blocks.
_P28_SKINNY_N128_DISJOINT_SIBLINGS = (
...
for _sibling_name, _sibling_set in _P28_SKINNY_N128_DISJOINT_SIBLINGS:
    assert _P28_SKINNY_N128_KCOMPL_ALIASSTACK_30.isdisjoint(_sibling_set), (
        f"P28 skinny_N128 (N=128) overlaps {_sibling_name}; "
        "sibling-N firewall violated — every disjoint K-COMPLEMENT sibling "
        "must use N ∉ {128} so the N=128 column is disjoint by construction. "
        "Intentional alias overlap with P13_N128 (the only N=128 sibling) "
        "is documented separately via `_P28_VS_P13_N128_ALIAS_OVERLAP`.")
del _sibling_name, _sibling_set
# Explicit P13_N128 alias overlap: P13_N128 routes all 18 cells in
# the P28 envelope at K ∈ {4096, 8192, 16384}; P13_N128 fires BEFORE
# P28 in the dispatch chain (7th-slot vs 19th-slot) so the P28
# membership check on those 18 cells is unreachable while P13_N128
# remains enabled.  The 18 alias cells are documentation; the load-
# bearing portion is the 6 NEW fp16 cells at K ∈ {2048, 32768}.
_P28_VS_P13_N128_ALIAS_OVERLAP = (
    _P28_SKINNY_N128_KCOMPL_ALIASSTACK_30
    & _K1367_P13_SKINNY_N128_KCOMPL_ROUTEOUT_18
)
assert _P28_VS_P13_N128_ALIAS_OVERLAP == frozenset({
    (M, 128, K, dtype)
    for M in (2048, 4096, 8192)
    for K in (4096, 8192, 16384)
    for dtype in ("torch.bfloat16", "torch.float16")
}), (
    "P28 vs P13_N128 alias overlap must be EXACTLY the 18 cells "
    "(M ∈ {2048,4096,8192} × N=128 × K ∈ {4096,8192,16384} × {bf16, fp16}); "
    "any deviation indicates either a P13_N128 contraction (re-audit P28 "
    "membership for the affected cells) or a P28 authoring typo against "
    "the verified admit set.")
# Explicit P5 closed-form alias overlap: P5 routes all 15 bf16
# cells in the P28 envelope (M ∈ {2048,4096,8192} × N=128 ≤ 128 ∧
# K ≥ 2048 satisfies the P5 admit predicate for dtype=bf16 on the
# closed-form structural pathology Gate-0).  P5 fires at the 4th-slot,
# well before P28 (19th).  Asserted as a function-call alias rather
# than a frozenset intersection because P5 is a closed-form predicate
# rather than a strict-equality table — the assert is performed at
# module load by enumerating the 15 bf16 cells and checking
# R_K979_P5_route_to_hbl returns True for each.
_P28_VS_P5_BF16_ALIAS = frozenset({
    (M, 128, K, "torch.bfloat16")
    for M in (2048, 4096, 8192)
    for K in (2048, 4096, 8192, 16384, 32768)
})
for _p28_p5_cell in _P28_VS_P5_BF16_ALIAS:
    _M_p5, _N_p5, _K_p5, _dtype_p5 = _p28_p5_cell
    assert R_K979_P5_route_to_hbl(_M_p5, _N_p5, _K_p5, _dtype_p5), (
        f"P28 vs P5 alias cell {_p28_p5_cell} not admitted by "
        "R_K979_P5_route_to_hbl; either the P5 closed-form predicate "
        "contracted or the P28 alias-decomposition rationale is "
        "stale — re-audit which cells P5 newly contributes vs the P28 "
        "envelope.")
del _p28_p5_cell, _M_p5, _N_p5, _K_p5, _dtype_p5
# ALIAS-STACK invariant: every cell in the P28 admit set MUST either
# (a) be covered by an upstream firing predicate (P5 closed-form for
# bf16 OR P13_N128 strict-equality for K ∈ {4096,8192,16384}) OR
# (b) be NEW route-OUT contributed by P28.  The (b) cells (the 6 NEW
# route-OUT fp16 cells) are the load-bearing portion; the (a) cells (24
# alias cells) are documentation that is unreachable under normal
# dispatch but freezes the 30-cell envelope under a single audit
# handle.  Total: 6 NEW + 24 alias = 30 admits.
_P28_NEW_ROUTEOUT_6 = (
    _P28_SKINNY_N128_KCOMPL_ALIASSTACK_30
    - _K1367_P13_SKINNY_N128_KCOMPL_ROUTEOUT_18
    - _P28_VS_P5_BF16_ALIAS
)
assert _P28_NEW_ROUTEOUT_6 == frozenset({
    (M, 128, K, "torch.float16")
    for M in (2048, 4096, 8192)
    for K in (2048, 32768)
}), (
    "P28 NEW route-OUT contribution must be exactly the 6 fp16 cells at "
    "K ∈ {2048, 32768} × M ∈ {2048, 4096, 8192} (dtype-symmetry gap "
    "closed); any deviation indicates either an upstream contraction "
    "(re-audit which cells P28 newly contributes) or a P28 authoring typo "
    "against the verified admit set.")
assert len(_P28_NEW_ROUTEOUT_6) == 6, (
    "P28 NEW route-OUT contribution must be exactly 6 cells "
    "(30 admits − 18 P13_N128 alias − 15 P5 bf16 alias + 9 P5∩P13_N128 "
    "double-counted = 6 NEW); any deviation indicates either an upstream "
    "contraction or a P28 authoring typo.")


def _p28_skinny_n128_kcompl_aliasstack_routeout(
    M: int, N: int, K: int, dtype) -> bool:
    """P28 — direct hipBLASLt route-OUT for the verified 30-cell
    skinny_N128 K-COMPLEMENT cohort
    (`_P28_SKINNY_N128_KCOMPL_ALIASSTACK_30`).
    ...
    MI300X / gfx942 against the LIVE post-P27 routing oracle.
    Engines: TB → tritonblas.matmul → persistent_matmul; HBL →
    hipBLASLt direct.  Pre-patch: 30/30 lose to hipBLASLt by ≥10%
    (cohort geomean 1.694×, worst 2.890× at (8192, 128, 2048, fp16));
    24 cells already routed-OUT by P5 ⨄ P13_N128; 6 fp16 mirror cells
    slip through every existing predicate at the K-EXTREMES sub-band —
    the load-bearing P28 contribution.

    ALIAS-STACK structure: 6 NEW cells (the fp16 mirror at K ∈ {2048,
    32768} that P5 + P13_N128 do not cover) + 24 alias cells (15 bf16
    alias-of-P5 + 18 K∈{4096,8192,16384} alias-of-P13_N128 with 9 cell
    P5∩P13_N128 intersection = 24 union).  Alias overlaps fire BEFORE
    P28 in the dispatch chain (P5 at 4th-slot, P13_N128 at 7th-slot)
    so the 24 alias cells are documentation; the 6 NEW cells are the
    load-bearing portion.

    Mechanism: PMC RCA reproduces the LDS-bank-conflict fingerprint
    bit-identically across dtypes — TB BC_cpi ∈ [1.4545, 2.133] vs
    HBL BC_cpi = 0.000 exactly across all 6 PMC cells; LDS-inst/wave
    inflation 2.41× at (2048, 128, 32768).  Dtype-invariance proof:
    bf16 row 1 and fp16 row 2 at the same MNK emit BIT-IDENTICAL TB
    BC_cpi (1.4545), TB LDS-inst/wave (2816), and TB cycles/wave
    within 0.1% — the LDS swizzle topology lives below the dtype lane
    mux so the routing predicate must mirror across dtype.  Why
    hipBLASLt wins: split-K / stream-K kernel selection vs tritonblas
    persistent_matmul which is L2/VMEM bound at N ≤ 256.
    CONFIG-FIXABLE → false at the level of BLOCK_K / NS / WPEU / NW
    retunes.
    ...
    after P26 (17th, N=2048) ⨄ P27 (18th, N=512 identity-alias of
    P23_N512); extends the alias-stack chain to dtype-symmetric admit
    sets.  Sibling-N firewall: every other K-COMPLEMENT sibling
    targets N ∉ {128}; intentional alias overlap with P13_N128 (the
    only N=128 predecessor) is documented separately via
    `_P28_VS_P13_N128_ALIAS_OVERLAP`.
    """
    return (
        (int(M), int(N), int(K), str(dtype))
