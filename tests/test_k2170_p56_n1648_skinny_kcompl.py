"""K-2170 (S-002) — invariants for P56 N=1648 K-COMPLEMENT alias-stack.
...
fix/K-2158@beab03e (which itself branches off fix/K-2127@9c4f984, fix/K-1922@0024a71 — matches
...
from tritonblas._route_predicate import (
    _K2170_P56_SKINNY_N1648_KCOMPL_ALIASSTACK_18,
)
...
def test_cardinality_is_18():
    assert len(_K2170_P56_SKINNY_N1648_KCOMPL_ALIASSTACK_18) == EXPECTED_CARDINALITY


def test_n_is_residue_48_mod_64():
    # Off-by-48 wave-misaligned membership invariant
    for (M, N, K, dt) in _K2170_P56_SKINNY_N1648_KCOMPL_ALIASSTACK_18:
        assert N == EXPECTED_N
        assert N % 64 == 48
        assert N % 128 == 112
...
def test_full_grid_membership():
    ...
    assert _K2170_P56_SKINNY_N1648_KCOMPL_ALIASSTACK_18 == expected


def test_dtype_set_is_bf16_fp16_only():
    dts = {dt for (_, _, _, dt) in _K2170_P56_SKINNY_N1648_KCOMPL_ALIASSTACK_18}
    assert dts == set(EXPECTED_DT)


def test_disjoint_with_prior_off_by_48_rungs():
    # K-2170 is a single-N slice at N=1648; every prior off-by-48 rung is at
    ...
    assert _K2170_P56_SKINNY_N1648_KCOMPL_ALIASSTACK_18.isdisjoint(
        _K1922_P40_SKINNY_N544_KCOMPL_ALIASSTACK_18
    )
    assert _K2170_P56_SKINNY_N1648_KCOMPL_ALIASSTACK_18.isdisjoint(
        _K2127_P54_SKINNY_N1456_KCOMPL_ALIASSTACK_18
...
def test_n1648_extends_off_by_48_ladder_above_n1584():
    ...
    # N=1584 in this rung sequence (next +64 contiguous step).
