#!/usr/bin/env python3
"""K-1429 P16 skinny_N1024 K-COMPLEMENT 30-cell paired benchmark (BASE ∪ EXTREMES).

Mirrors K-1409 P15 N=512 protocol exactly with N axis bumped to 1024 and the
K axis extended to the full BASE ∪ EXTREMES envelope:

- 30-cell cohort: M ∈ {2048,4096,8192} × N=1024 ×
                  K ∈ {2048, 4096, 8192, 16384, 32768} × {bf16, fp16}.
...
- ROUTED arm  = tritonblas.matmul (native triton path before this PR; this PR
                                   re-routes the cohort to hipBLASLt).
- HBL arm     = torch.matmul (PyTorch dispatches to hipBLASLt).
- ratio r     = tb_us / hbl_us ; r > 1 ⇒ hipBLASLt is faster ⇒ confirms
                                          the K-1429 productionisation.

Output JSON: per-cell median ratio, CI95-lo/hi, raw 30-sample arrays.
Reproduces the K-1429 upstream measurement and pins the K-1441 productionisation.

Run:
    python3 scripts/k1429_paired_n30.py \
        --out-json /home/ryaswann/mc2-workspaces/K-1441/output/k1429_paired_n30.json \
        --out-csv  /home/ryaswann/mc2-workspaces/K-1441/output/k1429_paired_n30.csv
"""
...
N_VALUE = 1024
K_VALUES = (2048, 4096, 8192, 16384, 32768)  # BASE ∪ EXTREMES (K-1429 unified envelope)
DTYPES = {
    "bf16": torch.bfloat16,
    "fp16": torch.float16,
}
...
def main() -> int:
    ...
    # Cohort geomean over all per-cell median ratios.
    medians = np.array([r.ratio_median for r in results], dtype=np.float64)
    geomean = float(np.exp(np.log(medians).mean()))
    print(f"[done] {len(results)} cells in {time.time()-t0:.1f}s "
          f"cohort_geomean(tb/hbl)={geomean:.3f}",
          file=sys.stderr, flush=True)
