...
Productionizes the verification of the N=128 K-COMPLEMENT envelope
...
Source measurement (still-of-record): paired n=30 HIP-graph hot-
...
def test_p28_admit_set_covers_full_n128_kcompl_grid():
    ...
    assert _P28_SKINNY_N128_KCOMPL_ALIASSTACK_30 == expected
...
#     upstream predicate must trip these tests so the NEW route-
...
def test_p28_p13_n128_alias_overlap_is_exactly_eighteen_cells():
    ...
    assert len(_P28_VS_P13_N128_ALIAS_OVERLAP) == 18


def test_p28_p5_bf16_alias_is_exactly_fifteen_cells():
    assert _P28_VS_P5_BF16_ALIAS == frozenset({
        (M, 128, K, "torch.bfloat16")
        for M in (2048, 4096, 8192)
        for K in (2048, 4096, 8192, 16384, 32768)
    })
    assert len(_P28_VS_P5_BF16_ALIAS) == 15
...
def test_p28_p5_alias_actually_admitted_by_p5_closed_form():
    ...
    for cell in _P28_VS_P5_BF16_ALIAS:
        M, N, K, dtype = cell
        assert R_K979_P5_route_to_hbl(M, N, K, dtype), (
            f"P28 vs R-K979 P5 alias cell {cell} not admitted by "
            "R_K979_P5_route_to_hbl; either the P5 closed-form predicate "
            "contracted or the P28 alias-decomposition rationale "
            "is stale.")
...
def test_p28_new_routeout_contribution_is_exactly_six_fp16_cells():
    ...
    for cell in _P28_NEW_ROUTEOUT_6:
        M, N, K, dtype = cell
        assert N == 128
...
def test_p28_disjoint_from_p26_n2048():
    assert _P28_SKINNY_N128_KCOMPL_ALIASSTACK_30.isdisjoint(
        _K1611_P26_SKINNY_N2048_KCOMPL_ALIASSTACK_30)
...
@pytest.mark.parametrize(
    "cell", sorted(_P28_SKINNY_N128_KCOMPL_ALIASSTACK_30)
)
def test_p28_admit_cell_routes_to_hbl(cell):
    ...
    assert (
        ...
        is True
    ), f"P28 admit cell {cell} failed to dispatch to hipBLASLt."
...
def test_p28_predicate_fires_on_admit_cell(cell):
    M, N, K, dtype = cell
    assert _p28_skinny_n128_kcompl_aliasstack_routeout(M, N, K, dtype)
...
def test_p28_predicate_does_not_fire_outside_envelope(cell):
    M, N, K, dtype = cell
    assert (
        _p28_skinny_n128_kcompl_aliasstack_routeout(M, N, K, dtype)
        is False
    )
...
def test_p28_admit_cell_carved_out_by_flags(kwargs):
    # canonical NEW admit (the worst-cohort fp16 cell).
...
#     upstream contraction silently breaks this, the audit handle
#     is no longer faithful and the alias cells become NEW route-OUT
#     (which would change the cardinality of `_P28_NEW_ROUTEOUT_6`).
# ---------------------------------------------------------------------------
def test_p28_alias_cells_covered_by_upstream_union():
    ...
    for cell in alias_cells:
        ...
        assert in_p13 or in_p5, (
            f"P28 alias cell {cell} not covered by either P13_N128 "
...
#     P28 strict-equality predicate must return False.  This is
...
def test_p28_does_not_fire_on_p26_n2048_cells():
    for cell in _K1611_P26_SKINNY_N2048_KCOMPL_ALIASSTACK_30:
        M, N, K, dtype = cell
        assert not _p28_skinny_n128_kcompl_aliasstack_routeout(
            M, N, K, dtype
        ), f"P28 fires on K-1611 P26 cell {cell} — sibling-N firewall violated."
