"""K-2197 (S-002) — invariants for P58 N=1776 K-COMPLEMENT alias-stack.
...
fix/K-2158@beab03e -> fix/K-2127@9c4f984 -> fix/K-1922@0024a71 — matches
K-2091 / K-2092 / K-2107 / K-2127 / K-2158 / K-2169 / K-2183 lineage).
Closes the K-2072 wide-N envelope cap (admit-eligible through N=1776).
"""
...
def test_cardinality_is_8():
    assert len(_K2197_P58_SKINNY_N1776_KCOMPL_ALIASSTACK_8) == EXPECTED_CARDINALITY


def test_n_is_residue_48_mod_64():
    # Off-by-48 wave-misaligned membership invariant
    for (M, N, K, dt) in _K2197_P58_SKINNY_N1776_KCOMPL_ALIASSTACK_8:
        assert N == EXPECTED_N
        assert N % 64 == 48
        # mod 128 == 112 is the K-1990/K-1994 / K-2055-2066 / K-2071-2091 /
        # K-2169 sub-class — admit identically per K-2150
        # R-K2150.MOD-64-IS-BINDING (5th mod-128=112 entry).
        assert N % 128 == 112
...
def test_full_grid_membership():
    ...
    assert _K2197_P58_SKINNY_N1776_KCOMPL_ALIASSTACK_8 == expected


def test_dtype_set_is_bf16_fp16_only():
    dts = {dt for (_, _, _, dt) in _K2197_P58_SKINNY_N1776_KCOMPL_ALIASSTACK_8}
    assert dts == set(EXPECTED_DT)


def test_disjoint_with_prior_off_by_48_rungs():
    # K-2197 is a single-N slice at N=1776; every prior off-by-48 rung is
    ...
    assert _K2197_P58_SKINNY_N1776_KCOMPL_ALIASSTACK_8.isdisjoint(
        _K1922_P40_SKINNY_N544_KCOMPL_ALIASSTACK_18
    )
    assert _K2197_P58_SKINNY_N1776_KCOMPL_ALIASSTACK_8.isdisjoint(
        _K2127_P54_SKINNY_N1456_KCOMPL_ALIASSTACK_18
...
def test_adjacency_firewalls_n1776_neighbors():
    ...
    # OUTSIDE the K-2197 frozenset — preserve the per-N audit handles per
    ...
    for cell in forbidden:
        assert cell not in _K2197_P58_SKINNY_N1776_KCOMPL_ALIASSTACK_8, (
            f"K-2197 frozenset must NOT contain firewall cell {cell}"
        )
