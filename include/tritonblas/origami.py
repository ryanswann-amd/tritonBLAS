...
#
# See PR notes: A/B sweep (300 shapes, fp16+bf16, MI300X) shows the override
# is roughly neutral (M=16 sub-cohort 0.303x -> 0.299x vs torch.matmul). It
# helps high-K (K=8192) modestly and slightly regresses low/mid-K. The kill
# switch is left in place so the override can be disabled without a code
# change while autotuning the small-M cohort further.
...
class OrigamiMatmulSelector:
    ...
    def __init__(
        ...
        # Origami's selector picks BLOCK_M=32 for M=16 shapes (smallest MN tile
        # in the default range that maximizes occupancy). With MFMA 16x16x{16,32}
        # on gfx942, BLOCK_M=32 means a single 32-row macro-tile contains the
        # only 16 real rows of A — half the MFMA M-lanes are masked, halving
        # effective compute throughput. K-198 attribution showed the residual
        # small-M gap is VALU-bound on padded MFMA (100% VALU, <1% MemBusy,
        # 0 LDS conflicts), so cutting BLOCK_M to 16 directly removes the
        # masked-row penalty without changing memory traffic. Gate strictly to
        # M<=16 so the M=32 sub-cohort (which uses the full 32-row macro-tile)
        # is untouched. (BLOCK_N=256, BLOCK_K=64) is the widest N tile that
        # keeps LDS within capacity and preserves the K-loop pipeline depth.
