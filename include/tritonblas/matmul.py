...
from ._route_predicate import (
    ...
    # K-COMPLEMENT alias-stack quad — N ∈ {608, 672, 704, 736}, 4 × 18 =
    # 72 cells total.  ONE 72-cell frozenset replacing what would be a
    # 4-probe chain of near-identical per-N frozensets (R-1881 minimalism).
    # Extends the K-1922 P40 N=544 promotion across the next four
    # K-COMPLEMENT N-buckets validated by K-1938 (N=608),
    # K-1923/K-1927/K-1931 (N=672), K-1945 (N=704), and K-1941/K-1943
    # (N=736) paired-n=30 HIP-graph hot-cache cohorts; cohort geomean
    # tb/hbl ≥ ~1.4× per rung, 0/72 regressions on the K-1925 231-cell
    # drift baseline.  K-1946 depth-4 closed-form predicate substitution
    # is held back pending K-1942-style bit-equiv revalidation on the
...
def _k971_route_to_hbl(M, N, K, a_dtype, b_dtype, enable_streamk, work_stealing):
    ...
    # K-1963 (S-002): consolidated P41–P44 wave-/tile-misaligned skinny-N
    # K-COMPLEMENT alias-stack quad — N ∈ {608, 672, 704, 736}, 4 × 18 =
    # 72 cells total.  ONE O(1) hash lookup replacing a 4-probe chain
    # (R-1881 minimalism — same data-driven consolidation pattern as the
    # K-1881 P32–P38 collapse); per-rung structural invariants pinned in
    # tests/test_k1963_p41_p44_skinny_n608_672_704_736_alias_stack.py and
    # bit-equivalence with the LIVE post-K-1922 oracle pinned in
    # test_oracle_bit_equivalence.  Per-rung cohort geomean tb/hbl ≥ ~1.4×;
    # sibling-N firewall disjoint with every prior K-COMPLEMENT alias-stack
    # slot.  Off-band fingerprint per rung (BLOCK_N=128 packing): N=608
    # mod 64=32 / mod 128=96, N=672 mod 64=32 / mod 128=32, N=704 mod 64=0
    # / mod 128=64 (BN-half-tile tail), N=736 mod 64=32 / mod 128=96.
