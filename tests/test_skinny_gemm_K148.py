"""Regression tests for the K-148 skinny-M GEMM heuristic.
...
pytestmark = pytest.mark.skipif(
    not torch.cuda.is_available(), reason="K-148 heuristic targets GPU shapes"
)
...
def test_compute_sk_grid_rollback_still_fires_when_uneven_dp_split():
    ...
    assert "sk_grid <= tiles and tiles % sk_grid != 0" in src, (
        "K-148 fix to _compute_sk_grid rollback predicate appears reverted"
    )
...
# ---------------------------------------------------------------------------
#
# The K-148 fix changes a predicate inside _compute_sk_grid, which is
# called for every streamk shape — not just M<=32. This test is the
# safety net: pick a representative M>32 shape and assert that
# tritonblas.matmul still matches torch within the same tolerance the
# baseline meets on main. Tolerance is bf16-realistic and matches what
