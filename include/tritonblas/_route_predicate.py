...
def k971_route_decision(M, N, K, a_dtype, b_dtype, enable_streamk,
    ...
    Precedence (top-down, first match wins):
      ...
      9. K-1412 P15 skinny_N512 K-COMPLEMENT 12-cell strict-equality -> hipBLASLt.
    """
    if disable_env_set:
        return False
    ...
    if _k1382_p14_skinny_n256_routeout(int(M), int(N), int(K), a_dtype):
        return True
    # K-1412 P15 (9th-position): skinny_N512 K-COMPLEMENT 12-cell route-OUT
    # at K-axis EXTREMES (K ∈ {2048, 32768}).  Stacks AFTER K-1382 P14 per
    # K-1175 stacked-predicate convention; closes the K-1404 post-P14 4-bucket
    ...
    # Mechanism: K=2048 starves persistent_matmul tile parallelism; K=32768
    # overflows N=512 LDS bank-conflict capacity (R-1412 inherits R-1397
    # extremes-not-mid-band rule with R-1409 N-axis attenuation).  Envelope
    # grows 91 → 103 cells.
    if _k1412_p15_skinny_n512_routeout(int(M), int(N), int(K), a_dtype):
        return True
    return False
