...
# Tuples where the route-OUT envelope intentionally routes to hbl despite
# the K-950 LAND verdict.  Sources:
#   * K-905/K-971 anchor table — historical strict-equality admits
#     (long-K square_mid; bf16+fp16 measurement overrides documented in
#     the K-905/K-971 PR descriptions).
#   * K-1361 P12 _K1295_P12_PMC_SQUARE_MID_ROUTEOUT_4 — productionizes the
#     K-1295/K-1345 square_mid 2048³/4096³ frozenset (see
#     `_route_predicate.py`); L05 (4096,4096,4096, bf16) is the bf16
#     anchor of the P12 4-cell admit (PMC-confirmed VALU/MFMA + LDS-BC
#     mechanism, K-1361 paired n=30 / n=100 backtest).
K950_ANCHOR_COLLISIONS = frozenset({
    (1024, 1024, 16384, "torch.bfloat16"),  # L07 — K-905/K-971 anchor
    (1024, 1024, 16384, "torch.float16"),   # L09 — K-905/K-971 anchor
    (2048, 2048, 16384, "torch.float16"),   # L06 — K-905/K-971 anchor
    (4096, 4096,  4096, "torch.bfloat16"),  # L05 — K-1361 P12 admit (square_mid 4096³ bf16)
})
