...
from tritonblas._route_predicate import (
    _K1538_P23_SKINNY_N512_KCOMPL_ROUTEOUT_N,
...
def test_frozenset_matches_expected_admit_set():
    assert _K1538_P23_SKINNY_N512_KCOMPL_ROUTEOUT_N == EXPECTED_ADMIT_30


def test_every_admit_cell_in_sweep_grid():
    for M, N, K, dtype_str in _K1538_P23_SKINNY_N512_KCOMPL_ROUTEOUT_N:
...
def test_disjoint_from_p8_mfma_envelope():
    assert _K1538_P23_SKINNY_N512_KCOMPL_ROUTEOUT_N.isdisjoint(
        _P8_MFMA_ISSUE_STALL_ROUTEOUT)


def test_disjoint_from_k971_route_table():
    assert _K1538_P23_SKINNY_N512_KCOMPL_ROUTEOUT_N.isdisjoint(K971_ROUTE_TABLE)
    ...
    for M in (2048, 4096, 8192):
        for K in (2048, 4096, 8192, 16384, 32768):
            for dt_str in ("torch.bfloat16", "torch.float16"):
                assert (M, 512, K, dt_str) in _K1538_P23_SKINNY_N512_KCOMPL_ROUTEOUT_N
