...
# Add include path for tritonblas module
...
def get_full_tuning_space(num_cus):
    ...
    chunk_size_range = [32]  # Default chunk size for full tuning
    ...
    space = itertools.product(block_mn_range, block_mn_range, block_k_range, num_warps_range, group_m_range,
                              ...
                              kpack_range, chunk_size_range)

    for instance in space:
        block_m, block_n, block_k, num_warps, group_m, num_sms, num_stages, waves_per_eu, matrix_instr_nonkdim, kpack, chunk_size = instance
        configs.append({
...
def get_tuning_space_with_selector(M, N, K, dtype_a, dtype_b, dtype_c, num_cus):
    """
    ...
    for instance in space:
        ...
        configs.append({
            ...
            'BLOCK_SIZE_K': BLK_K,      # From selector
            'GROUP_SIZE_M': gsize_m,    # From selector
            'NUM_SMS': num_cus,         # From hardware
            'num_warps': num_warps,     # TUNE
            'num_stages': num_stages,   # TUNE
            'waves_per_eu': waves_per_eu,  # TUNE
            'matrix_instr_nonkdim': matrix_instr_nonkdim,  # TUNE
            'kpack': kpack,             # TUNE
...
def profile_batch_kernels(M, N, K, gpuid, gpus, jobs, verbose):
    ...
    while jobId < jobs:
        ...
        run_bash_command_wrapper(
            f"rocprofv3 --kernel-trace -f csv -o {get_output_dir()}/{jobId} --log-level fatal -- python {get_filename_profile_driver(M, N, K, jobId)}",
            capture=(verbose < 2))
        jobId += ngpus


def tune_gemm_config(M, N, K, col_a, col_b, dtype_a, dtype_b, dtype_c, dtype_p, dtype_lock, init_type, configs,
                     run_bench, jobs, iters, skipWarmup, module_name, kernel_name, kernel_type='streamk',
                     verbose=0, num_threads=32, gpus=[0], rotating_buffer_size=256, bias_size=0, icache_flush=False):
    ...
    if not skipWarmup:
        ...
        fname = generate_compile_driver(M, N, K, col_a, col_b, dtype_a, dtype_b, dtype_c, dtype_p, dtype_lock,
                                        init_type, configs, rotating_buffer_size, bias_size, kernel_name, kernel_type)
...
def parse_args():
    ...
    parser.add_argument('--kernel', default='tritonblas.internal.streamk_matmul, streamk_matmul',
                        help='Module name and kernel name separated by comma. '
                             'Examples: tritonblas.internal.streamk_matmul,streamk_matmul or '
                             'tritonblas.internal.persistent_matmul,persistent_matmul')
    parser.add_argument('--kernel_type', type=str, default='auto',
                        choices=['auto', 'streamk', 'persistent'],
                        help='Kernel type - auto-detect from module name or specify explicitly')
    parser.add_argument('--use_selector', action='store_true', default=False,
                        help='Use origami selector to determine tile sizes (dramatically reduces tuning space)')
...
def main():
    ...
    if args.kernel_type == 'auto':
        ...
        elif 'streamk' in module_name.lower():
            kernel_type = 'streamk'
        else:
            print(f"Warning: Cannot auto-detect kernel type from '{module_name}', defaulting to streamk")
            kernel_type = 'streamk'
    else:
    ...
    if not args.o:
        if args.benchmark:
            args.o = f"benchmarking_results_{kernel_name}_{kernel_type}.csv"
        else:
            args.o = get_default_tuning_result_filename(f"{kernel_name}_{kernel_type}")
    ...
    if args.compare_wo_tuning:
        for (M, N, K, col_a, col_b, myConfig) in mnks:
            ...
            test_correctness(kernel_func, M, N, K, col_a, col_b, dtype_a, dtype_b, dtype_c, init_type, myConfig,
                             bias_vector, True, kernel_type)
        return

    # Prepare full tuning space if not using selector
    configs_full = None if args.use_selector else get_full_tuning_space(num_cus)
    ...
    for (M, N, K, col_a, col_b, myConfig) in mnks:
        ...
        if run_bench:
            ...
            # Traditional full-space tuning
            pruned_configs = prune_configs(M, N, K, configs_full, type_name_to_bytes(dtype_a),
        ...
        minTime, bestConfig, compile_time, profile_time, post_time = tune_gemm_config(
            ...
            jobs, iters, skipWarmup, module_name, kernel_name, kernel_type,
        ...
        if args.compare:
            ...
            test_correctness(kernel_func, M, N, K, col_a, col_b, dtype_a, dtype_b, dtype_c, init_type, bestConfig, bias_vector,
                             False, kernel_type)
        elif not run_bench:
            print("", flush=True)
