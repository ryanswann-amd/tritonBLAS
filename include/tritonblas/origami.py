...
class OrigamiMatmulSelector:
    ...
    def _compute_sk_grid(self):
        """
        Continuous StreamK grid heuristic.
        ...
        Workspace and last-wave fallbacks are preserved. The previous
        unconditional cleanup ``if tiles % sk_grid != 0: sk_grid = tiles``
        — which zeroed out the split on any remainder, ignoring its own
        comment about workspace — is corrected so the reset to ``tiles``
        only fires when the remainder also exceeds the workspace budget.
        """
        MIN_ITERS_PER_CU = 8
        ...
        # Cleanup-bug fix: only fall back to ``tiles`` (no split) when the
