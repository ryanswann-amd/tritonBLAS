...
Computes C[b] = A[b] @ B[b] for b in 0..BATCH-1 in a single grid launch
that schedules workgroups across (batch, M_tiles, N_tiles).  This replaces
the host-side Python loop ``[matmul(a[i], b[i]) for i in range(batch)]``
which serialised launches and prevented the GPU from overlapping batches.

Design notes:
- Grid is 1-D of length ``BATCH * M_tiles * N_tiles``.  ``pid`` decodes to
  ``(pid_b, pid_m, pid_n)`` in row-major order (batch outermost).  This
  keeps the pid-to-tile arithmetic identical to the single-shot kernel
  (and reuses the same group-M reordering for L2 locality) while letting
  the batch dim be a constexpr-light runtime int.
- Per-batch ptrs are advanced via ``stride_ab``, ``stride_bb``, ``stride_cb``.
  The kernel does NOT assume contiguous batches; strides are passed in.
- Tile shape comes from the standard Origami selector for (M, N, K).
"""
...
def batched_matmul(
    ...
    Grid = (BATCH * cdiv(M, BLOCK_M) * cdiv(N, BLOCK_N),).  Each program
    computes one (batch, M_tile, N_tile) output tile.
    """
    pid = tl.program_id(0)
    ...
    # Advance base pointers to the current batch slice.
