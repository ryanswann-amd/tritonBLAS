...
from ._route_predicate import (
    ...
    # 18-cell envelope (M ∈ {2048,4096,8192} × N=944 ×
    # K ∈ {4096,8192,16384} × {bf16,fp16}).  Off-by-48 wave-misaligned rung
    # (944 mod 64 = 48, mod 128 = 48); K-2010 paired n=30 HIP-graph hot-cache
    # MI300X / gfx942 vs live post-K-1922 oracle: BEFORE/AFTER cohort
    # geomean tb/hbl flips 1.365× → 1.005× (1.358× uplift, 17/18 ≥1.10×
    # admit, 16/18 strict flip BEFORE≥1.10→AFTER≤1.05).  Mirrors the K-1978
    # P45 N=816 and K-1994 P45 N=880 off-by-48 ladder rungs.  Naturally
...
def _k971_route_to_hbl(M, N, K, a_dtype, b_dtype, enable_streamk, work_stealing):
    ...
    # K-2010 (S-002): P46 (36th-slot) skinny_N944 K-COMPLEMENT alias-stack — 18 cells
    # (off-by-48 wave-misaligned rung; BEFORE/AFTER cohort geomean tb/hbl 1.365× → 1.005×,
    # 1.358× uplift, 17/18 ≥1.10× admit, 16/18 strict flip; mirrors K-1978 P45 N=816 /
    # K-1994 P45 N=880 off-by-48 ladder).
    if (int(M), int(N), int(K), str(a_dtype)) in _K2010_P46_SKINNY_N944_KCOMPL_ALIASSTACK_18: return True
    return False
