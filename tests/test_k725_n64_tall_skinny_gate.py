...
# --- IN-COHORT positives (the 18 cells that fired in the K-725 verify) -----

# Per the K-725 verify CSV (zero-regression tightened envelope):
_IN_COHORT_CELLS = [
    (16384, 1024), (16384, 2048), (16384, 4096), (16384, 8192),  # M>=16384, K>=1024
    (4096,  4096), (4096,  8192),                                # M>=4096,  K>=4096
    (8192,  4096), (8192,  8192),
    (2048,  8192),                                               # M>=2048,  K>=8192
]

@pytest.mark.parametrize("M,K", _IN_COHORT_CELLS)
@pytest.mark.parametrize("dtype", _FP_DTYPES)
def test_in_cohort_fires(M, K, dtype):
    """Every (M, K) cell that won >=1.20x in the K-725 verify must fire."""
...
def test_M_4096_K_4096_boundary_inclusive():
    """Lowest-M corner of the second disjunct: M=4096, K=4096."""
    for dtype in _FP_DTYPES:
        assert _is_k725_n64_tall_skinny(4096, 64, 4096, dtype) is True
