...
def test_medium_k_residual_tile_count_upper_bound_excludes_large_grids():
    """Large-square PRD-guard losers: tiles > 128 -> reject (L2 working-set spill)."""
...
def test_medium_k_residual_admits_winner_envelope():
    """Original winner cohort: K/BK=32 AND tiles=128 -> admit."""
    # All three original winner shapes at the autotuner-selected BM=BN=256, BK=64.
    for M, N, K in [(4096, 2048, 2048), (8192, 1024, 2048), (2048, 4096, 2048)]:
        ...
        ), f"winner shape {M}x{N}x{K} must remain admitted"
...
def test_on_mode_returns_swizzled_only_inside_envelope(monkeypatch):
    """mode=on respects the envelope.

    Previously, mode=on returned SWIZZLED unconditionally -- that contract
    landed the original kpack=2 knob despite later-discovered regressions
    on PRD-guard shapes and a small-K cohort (all triggered when downstream
    consumers forced mode=on, bypassing the gate). Mode=on must now match
    the autotuned `auto`-mode envelope.
    """
    monkeypatch.setenv("TRITONBLAS_LDS_SWIZZLE", "on")
    # In-envelope winner shape -> swizzled.
    assert _select(M=4096, N=2048, K=2048, block_m=256, block_n=256, block_k=64) \
        == SWIZZLED_CONFIG
    # Out-of-envelope small-K cohort -> baseline (was buggy SWIZZLED previously).
...
def test_autotune_picks_faster_and_persists(monkeypatch, tmp_path):
    ...
    # Use an in-envelope winner shape (BM=BN=256, BK=64, K=2048) so the
...
def test_autotune_skips_swizzled_outside_envelope(monkeypatch, tmp_path):
    ...
    The small-K regression root cause was an autotune cache poisoned with
    kpack=2 entries on shapes outside the calibrated envelope; this test
    pins the contract that autotune restricts its candidate set to BASELINE
    for such shapes.
    """
...
def test_cached_kpack2_dropped_for_out_of_envelope_shape(monkeypatch, tmp_path):
    """Stale kpack=2 cache entries on out-of-envelope shapes are dropped to
    baseline at lookup time, not served.

    This protects against silent regressions when the envelope tightens
    between releases (the cache-poisoning mechanism).
    """
