...
from ._route_predicate import (
    ...
    # (mod-64=48).  Disjoint with every prior slot (no shared N).
    # Branched off fix/K-1922@0024a71.
...
def _k971_route_to_hbl(M, N, K, a_dtype, b_dtype, enable_streamk, work_stealing):
    ...
    # K-2139 (S-002): P54 skinny_N1520 K-COMPLEMENT alias-stack — next +64
    # contiguous rung above K-2127 P54 N=1456 in the off-by-48 ladder
    # (816/880/944/1008/1072/1136/1200/1264/1328/1392/1456 → 1520).  18 cells
    # (M ∈ {2048,4096,8192} × N=1520 × K ∈ {4096,8192,16384} × {bf16,fp16}).
    # Same off-by-48 wave-misalignment mechanism as every prior rung
    # (BLOCK_N=128 → 11.875 fractional tiles per N-row).
    if (int(M), int(N), int(K), str(a_dtype)) in _K2139_P54_SKINNY_N1520_KCOMPL_ALIASSTACK_18: return True
    return False
