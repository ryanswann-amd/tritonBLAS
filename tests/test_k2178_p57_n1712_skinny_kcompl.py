"""K-2178 (S-002) — invariants for P57 N=1712 K-COMPLEMENT alias-stack.
...
def test_cardinality_is_8():
    assert len(_K2178_P57_SKINNY_N1712_KCOMPL_ALIASSTACK_8) == EXPECTED_CARDINALITY


def test_n_is_residue_48_mod_64():
    # Off-by-48 wave-misaligned membership invariant
    for (M, N, K, dt) in _K2178_P57_SKINNY_N1712_KCOMPL_ALIASSTACK_8:
        assert N == EXPECTED_N
        assert N % 64 == 48
        # mod 128 == 48 is the K-1978 / K-2071 / K-2150 / K-2158 sub-class —
...
def test_full_grid_membership():
    ...
    assert _K2178_P57_SKINNY_N1712_KCOMPL_ALIASSTACK_8 == expected


def test_dtype_set_is_bf16_fp16_only():
    dts = {dt for (_, _, _, dt) in _K2178_P57_SKINNY_N1712_KCOMPL_ALIASSTACK_8}
    assert dts == set(EXPECTED_DT)


def test_disjoint_with_prior_off_by_48_rungs():
    # K-2178 is a single-N slice at N=1712; every prior off-by-48 rung is at
    # a strictly different N (544..1648) so disjointness must hold trivially.
    from tritonblas._route_predicate import (
    ...
    assert _K2178_P57_SKINNY_N1712_KCOMPL_ALIASSTACK_8.isdisjoint(
        _K1922_P40_SKINNY_N544_KCOMPL_ALIASSTACK_18
    )
    assert _K2178_P57_SKINNY_N1712_KCOMPL_ALIASSTACK_8.isdisjoint(
        _K2127_P54_SKINNY_N1456_KCOMPL_ALIASSTACK_18
