...
def k971_route_decision(M, N, K, a_dtype, b_dtype, enable_streamk,
    ...
    if _k1673_p28_skinny_n128_kcompl_aliasstack_routeout(
            int(M), int(N), int(K), a_dtype):
        return True
    # K-1720 P30 (20th): skinny_Nmid (N ∈ {384, 768, 1536}) K-COMPLEMENT
    # 34-cell route-OUT.  Closes the K-1711 cross-N envelope-stitching
    # gap (cohort geomean 1.456×).
...
# route-OUT (20th-position).  Closes the K-1711 cross-N envelope-stitching
# gap at the in-between-N values N ∈ {384, 768, 1536} that fall between
# the productionized covered N-bins {128, 256, 512, 1024, 2048, 4096}.
# Source: K-1711 paired n=30 HIP-graph hot-cache MI300X gfx942 vs the LIVE
# post-K-1685 P28 oracle; 34/36 CI95-gated flagged cells (cohort geomean
# 1.456×, range 1.18×-1.83×).  Per-N admit shapes (10 / 14 / 10) and the
# sibling-N firewall vs P1-P28 are pinned by the K-1720 unit suite —
# `tests/test_k1720_p30_skinny_nmid_alias_stack.py` (no parallel src-side
# data structures, per R-1532 minimalist-admit-set).
# ---------------------------------------------------------------------------
_K1720_P30_SKINNY_NMID_KCOMPL_ALIASSTACK_34 = frozenset({
    # N=384 (10 cells)
    (2048,  384,  8192, "torch.float16"),    (2048,  384,  8192, "torch.bfloat16"),
    (2048,  384, 32768, "torch.float16"),    (2048,  384, 32768, "torch.bfloat16"),
    (4096,  384,  8192, "torch.float16"),    (4096,  384,  8192, "torch.bfloat16"),
    (4096,  384, 32768, "torch.float16"),    (4096,  384, 32768, "torch.bfloat16"),
    (8192,  384, 32768, "torch.float16"),    (8192,  384, 32768, "torch.bfloat16"),
    # N=768 (14 cells, worst seam)
    (2048,  768,  8192, "torch.float16"),    (2048,  768,  8192, "torch.bfloat16"),
    (2048,  768, 32768, "torch.float16"),    (2048,  768, 32768, "torch.bfloat16"),
    (4096,  768,  8192, "torch.float16"),    (4096,  768,  8192, "torch.bfloat16"),
    (4096,  768, 32768, "torch.float16"),    (4096,  768, 32768, "torch.bfloat16"),
    (8192,  768,  2048, "torch.float16"),    (8192,  768,  2048, "torch.bfloat16"),
    (8192,  768,  8192, "torch.float16"),    (8192,  768,  8192, "torch.bfloat16"),
    (8192,  768, 32768, "torch.float16"),    (8192,  768, 32768, "torch.bfloat16"),
    # N=1536 (10 cells)
    (2048, 1536, 32768, "torch.float16"),    (2048, 1536, 32768, "torch.bfloat16"),
    (4096, 1536,  2048, "torch.float16"),    (4096, 1536,  2048, "torch.bfloat16"),
    (4096, 1536,  8192, "torch.float16"),    (4096, 1536,  8192, "torch.bfloat16"),
    (4096, 1536, 32768, "torch.float16"),    (4096, 1536, 32768, "torch.bfloat16"),
    (8192, 1536, 32768, "torch.float16"),    (8192, 1536, 32768, "torch.bfloat16"),
})
...
def _k1720_p30_skinny_nmid_kcompl_aliasstack_routeout(
    ...
    See `tests/test_k1720_p30_skinny_nmid_alias_stack.py` for the
    cardinality / per-N shape / sibling-N firewall / zero-alias invariants.
    """
    return (
        (int(M), int(N), int(K), str(dtype))
