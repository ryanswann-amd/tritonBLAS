"""K-2060 P49 (S-002) — 39th-slot N=1136 K-COMPLEMENT verified-winner alias-stack tests.

Source measurement: K-2060 paired n=30 HIP-graph hot-cache MI300X / gfx942
(vs the live post-K-2047 oracle, HEAD = 1d0b74c) over the 18-cell sub-cohort
M ∈ {2048, 4096, 8192} × N=1136 × K ∈ {4096, 8192, 16384} × {bf16, fp16}.

K-2060 promotes N=1136 as the 6th confirmed wave-misaligned rung in the
off-by-48 (mod 64) residue family above:
  ...
  - K-2060 P49 N=1136 (this rung; mod 128 = 112 — alternate-tail branch)
...
BLOCK_N=128 packs N=1136 into 8.875 tiles per N-row (1136 mod 128 = 112
= 7/8-tile tail).  1136 mod 64 = 48 places it in the off-by-48 wave-
misaligned class — the SIMD lane mismatch that has driven every prior rung's
admission.  K-2060 N=1136 shares the (mod 128 = 112) BLOCK_N tail with the
K-2031 P47 N=1008 rung (the alternate tail topology in the off-by-48
family); K-1978/K-1994/K-2014/K-2047 share the (mod 128 = 48) tail.  Per
K-1908, the existing S1-form `(N % 64 != 0) ∧ (N <= 384) ∧ (K >= 4096)`
predicate does NOT cover N=1136 (N=1136 > 384 — the predicate's N-ceiling
cuts off below this rung); explicit alias-stack promotion is required.
...
R-1720 / R-1775):
  ...
  5. Sibling-N firewall vs every prior K-COMPLEMENT alias-stack at a
     ...
     384}, P40 N=544, P48 N=1072).
  6. Off-by-48 wave-misalignment band membership: 1136 mod 64 == 48
     (same wave class as K-1978/K-1994/K-2014/K-2031/K-2047 rungs).
"""
...
FZ = _K2060_P49_SKINNY_N1136_KCOMPL_ALIASSTACK_18
...
def test_dtype_row_balance():
    ...
    K-1994 P45 N=880, K-2014/2020/2024 P46 N=944, K-2031 P47 N=1008,
    and K-2047 P48 N=1072)."""
...
def test_envelope_equals_full_n1136_kcompl_grid():
    ...
    detect any silent contraction or expansion of the K-2060 verification
    envelope."""
    full = frozenset(
...
def test_off_by_48_wave_misalignment_band_membership():
    ...
    P46 N=944, K-2031 P47 N=1008, and K-2047 P48 N=1072.  N=1136 mod 128
    == 112 — shares the alternate tail topology with K-2031 P47 N=1008
    (vs the canonical mod-128 = 48 tail of N=816/880/944/1072).  This
    invariant guards against a silent N-axis typo (e.g. 1128, 1144, 1152)
    that would land on a different SIMD wave alignment and thus reflect
...
def test_sibling_n_firewall_vs_consolidated_p32_p38():
    ...
    352, 384}; K-2060 P49 covers N=1136 — disjoint by natural N-axis
    separation."""
    assert FZ & _K1881_P32_P38_SKINNY_KCOMPL_ROUTEOUT_120 == set()


def test_sibling_n_firewall_vs_p40_n544():
    """K-1922 P40 covers N=544; K-2060 P49 covers N=1136 — disjoint by
...
def test_sibling_n_firewall_vs_p48_n1072():
    """K-2047 P48 covers N=1072; K-2060 P49 covers N=1136 — disjoint by
    natural N-axis separation (1136 = 1072 + 64, next rung in off-by-48
    family).  This firewall is strictly load-bearing because P48 is the
    immediate predecessor in the off-by-48 ladder."""
