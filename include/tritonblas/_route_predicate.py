...
# per the minimalist split: src holds data, tests hold invariants
# (R-1532 / R-1720 / R-1775).
# K-2197 (S-002): P58 (next-position) skinny_N1776 K-COMPLEMENT alias-stack —
...
# 12.375).  Per K-2150 R-K2150.MOD-64-IS-BINDING-MOD-128-IS-NOT-FOR-
# RESIDUE-48-FAMILY only `mod 64 == 48` binds — the `mod 128 == 112`
# subclass at N=1776 admits identically (validated upstream at K-1990/
# K-1994 N=880 / K-2055-2066 N=1136 / K-2071-2091 N=1264 / K-2169 N=1648 —
# all `mod 128 == 112`; this is the 5th mod-128=112 entry).
#
# Mechanism (R-1811 wave-misalignment + K-913 §3 LDS-bank-conflict): same
# SCHEDULER_LDS A4 failure mode as every prior off-by-48 rung; per K-2061
# PMC delta-attribution residue-48 carries a stable LDS bank-conflict
# differential (~6×) vs residue-32 (~2×) and wave-aligned (~1×) at
# (M=4096, K=8192, bf16).  K-2197 paired n=30 HIP-graph hot-cache 3-engine
# bench (tb / hbl / rocblas) on the task-specified 8-cell verification
# cohort (M ∈ {4096,8192} × N=1776 × K ∈ {8192,16384} × {bf16,fp16})
# extends the K-2183 14-rung wall-clock chain by one residue-48 step
# (+64 from N=1712 on the same residue-48 lattice — closes the K-2072
# wide-N envelope cap).  The frozenset literal cardinality (==8) matches
# the measured verification cohort, not the canonical 18-cell envelope —
# extension to the full grid is deferred to a follow-up rung-stitch task.
#
# Naturally disjoint with every prior K-COMPLEMENT alias-stack slot (no
# shared N).  +2 active LOC additive in the dispatcher (1 import in
...
# prior off-by-48 rungs already in the alias-stack.
_K2197_P58_SKINNY_N1776_KCOMPL_ALIASSTACK_8 = frozenset(
    (M, 1776, K, dt) for M in (4096, 8192)
    for K in (8192, 16384) for dt in ("torch.bfloat16", "torch.float16")
)
# Cardinality (==8) gated by tests/test_k2197_p58_n1776_skinny_kcompl.py
# per the minimalist split: src holds data, tests hold invariants
# (R-1532 / R-1720 / R-1775).
