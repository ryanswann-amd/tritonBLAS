...
# ---------------------------------------------------------------------------
# K-1442 (S-002) — P17 `skinny_N2048` K-COMPLEMENT route-OUT envelope sweep
# (11th-position).  Productionises the K-1438-derived 20-cell K-COMPLEMENT
# envelope at N=2048 (21/30 sweep ADMIT; 1 admit cell deferred to P12 to
# preserve the K-1175 / R-1329 pairwise-disjointness invariant).
#
# K-1442 extends the K-1367 → K-1397 → K-1409 → K-1417 → K-1433 K-COMPLEMENT
# route-OUT lineage one bucket up the N axis (N=1024 → N=2048).  Unlike
# K-1433 (BASE-only, K ∈ {4096, 8192, 16384}), K-1442 sweeps the full
# BASE+EXTREMES K band (K ∈ {2048, 4096, 8192, 16384, 32768}) in a single
# 30-cell envelope and admits the 21 cells that pass the strict
# K-1442 admit gate (ratio_median ≥ 1.10 ∧ bootstrap p(<1.10) < 0.01).
#
# Cohort definition (sweep): M ∈ {2048, 4096, 8192} × N=2048 ×
#   K ∈ {2048, 4096, 8192, 16384, 32768} × {bf16, fp16} = 30 cells.
# Admitted by K-1442 sweep (route-OUT to hipBLASLt): 21 / 30.
# Admitted into the P17 frozenset: 20 / 30.  The 21st admit cell
# (2048, 2048, 2048, fp16) — r=1.121, CI99-lo=1.100, p(<1.10)=0.0049 —
# is dropped from the frozenset because K-1295 P12 square_mid already
# routes it at the 6th position; routing behaviour for that key is
# unchanged but the K-1175 / R-1329 pairwise-disjointness invariant is
# preserved.
# Rejected (fall through to native triton — TB persistent_matmul competitive):
#   * (2048, 2048,  2048, bf16) r=1.106  — small-M canary, p(<1.10)=0.16
#   * (2048, 2048,  4096, bf16) r=1.049
#   * (2048, 2048,  8192, bf16) r=1.039
#   * (2048, 2048, 16384, bf16) r=1.021
#   * (2048, 2048, 16384, fp16) r=1.014
#   * (2048, 2048, 32768, bf16) r=1.012
#   * (2048, 2048, 32768, fp16) r=1.009
#   * (8192, 2048,  8192, fp16) r=1.033  — single mid-K M=8192 fp16 hole
#   * (8192, 2048, 32768, bf16) r=1.004  — large-K M=8192 bf16 hole
#
# PMC mechanism (consistent with K-913 / K-1397 / K-1409 / K-1433):
#   At N=2048 the column-narrow regime widens further; the LDS-bank-conflict
#   discriminator that drives the K-COMPLEMENT win attenuates on the small-M
#   row (M=2048) where TB persistent_matmul tile geometry no longer over-
#   serialises bank loads, but remains dominant on M ∈ {4096, 8192} rows
#   where wider M still saturates the LDS BC mechanism.  The two M=8192
#   rejects sit at the K-axis crossover where TB's split-K kernel selection
#   matches hipBLASLt's within measurement noise.
#
# Why N=2048 K-edges still favour hipBLASLt on M ∈ {4096, 8192}:
#   * L2 / LDS pressure crossover — at N=2048 the per-CU output tile (32×128
#     to 64×128 typical for persistent_matmul) sweeps a wider K-strip, and
#     the per-CTA LDS load pattern at K ∈ {4096, 8192} still triggers the
#     K-913 §3 LDS-bank-conflict signature.  hipBLASLt's split-K + altered
#     tile geometry sidesteps the conflict at the K-mid band.
#   * At K ∈ {16384, 32768} the per-CTA accumulator-rotation cost rises and
#     hipBLASLt's heuristic switches to a different kernel family (split-K /
#     stream-K hybrids) that K-913 §3 does not penalise — TB's persistent
#     wave continues to overlap LDS conflicts with main-loop compute,
#     keeping the discriminator open for M ∈ {4096, 8192} but closing it
#     at M=8192 / K=32768 / bf16.
#
# Verification (paired n=30 HIP-graph hot-cache, B=10000 vectorised paired
# bootstrap on per-iteration ratio array — an OCI MI300X fallback host
# (gfx942), c42 down):
#   21/30 sweep ADMIT (every admitted cell passes ratio_median ≥ 1.10 ∧
#                     bootstrap p(<1.10) < 0.01)
#   20/30 productionised into this frozenset (canary fp16 cell deferred
#                     to P12 — see disjointness section below)
#   cohort geomean tb/hbl over the 20 productionised cells = **1.335×**
#                                                            (range 1.142×–1.690×)
#   min CI99-lo            = **1.105** at (8192, 2048, 32768, fp16) — canary
#   max bootstrap p(<1.10) = **0.0002** at the same canary cell
#   max ratio_median       = **1.690×** at (4096, 2048, 4096, bf16)
#   envelope grows         115 → 135 cells (10 → 11 predicate stack)
#
# Disjointness rationale (verified by frozenset.isdisjoint at module load):
#   * P8 sub-frozensets — no N=2048 cells in any P8 sub-frozenset
#     (K-1219 uses N=256, K-1205 uses N=128, others use M=N square shapes;
#     the only square-N=2048 form is M=N=2048 which P17 admits exclude
#     for bf16 — only P17 fp16 (2048,2048,2048,fp16) is in the frozenset
#     and is disjoint from P8 anchors which are bf16-only at that key).
#   * K971_ROUTE_TABLE — K-905/K-971/K-1335 anchors use M=N ∈ {1024, 2048}
#     with K ∈ {16384, 32768}; P17 (2048,2048,2048,fp16) has K=2048 (no
#     collision); P17 has no other M=N=2048 cells.
#   * K-1361 P12 — uses M=N=K ∈ {2048, 4096}; only P17 cell with M=N=K is
#     (2048,2048,2048,fp16); P12 covers bf16 there, fp16 is free.
#   * K-1367 P13 — uses N=128.
#   * K-1397 P13 — uses N=256.
#   * K-1417 P15 — uses N=512.
#   * K-1433 P16 — uses N=1024.
# All asserted at module load.
# ---------------------------------------------------------------------------
_K1438_P17_SKINNY_N2048_KCOMPL_ROUTEOUT_N = frozenset({
    # M=2048 row × {K, dtype} cells from K-1442 P17 sweep.
    # NOTE: (2048,2048,2048,fp16) was admitted by the K-1442 sweep
    # (r=1.121, CI99-lo=1.100, p(<1.10)=0.0049) but is NOT included here
    # because it is already routed by K-1295 P12 (square_mid 4-cell
    # `_K1295_P12_PMC_SQUARE_MID_ROUTEOUT_4` — M=N=K∈{2048,4096} bf16+fp16).
    # P12 fires at the 6th position, ahead of this 11th-position predicate,
    # so the routing behaviour is unchanged; dropping the cell from this
    # frozenset preserves the K-1175 / R-1329 pairwise-disjointness invariant
    # asserted at module load (see _K1442_P17_VS_P12_DISJOINT below).
    (2048, 2048,  4096, "torch.float16"),   # r=1.281 CI99-lo=1.263 p(<1.10)=0.0000
    (2048, 2048,  8192, "torch.float16"),   # r=1.240 CI99-lo=1.227 p(<1.10)=0.0000
    # M=4096 row × {K, dtype} cells from K-1442 P17 sweep (10/10 admit)
    (4096, 2048,  2048, "torch.bfloat16"),  # r=1.618 CI99-lo=1.538 p(<1.10)=0.0000
    (4096, 2048,  2048, "torch.float16"),   # r=1.564 CI99-lo=1.509 p(<1.10)=0.0000
    (4096, 2048,  4096, "torch.bfloat16"),  # r=1.690 CI99-lo=1.598 p(<1.10)=0.0000
    (4096, 2048,  4096, "torch.float16"),   # r=1.594 CI99-lo=1.572 p(<1.10)=0.0000
    (4096, 2048,  8192, "torch.bfloat16"),  # r=1.605 CI99-lo=1.571 p(<1.10)=0.0000
    (4096, 2048,  8192, "torch.float16"),   # r=1.561 CI99-lo=1.543 p(<1.10)=0.0000
    (4096, 2048, 16384, "torch.bfloat16"),  # r=1.191 CI99-lo=1.190 p(<1.10)=0.0000
    (4096, 2048, 16384, "torch.float16"),   # r=1.186 CI99-lo=1.185 p(<1.10)=0.0000
    (4096, 2048, 32768, "torch.bfloat16"),  # r=1.276 CI99-lo=1.275 p(<1.10)=0.0000
    (4096, 2048, 32768, "torch.float16"),   # r=1.289 CI99-lo=1.285 p(<1.10)=0.0000
    # M=8192 row × {K, dtype} cells from K-1442 P17 sweep (8/10 admit)
    (8192, 2048,  2048, "torch.bfloat16"),  # r=1.352 CI99-lo=1.337 p(<1.10)=0.0000
    (8192, 2048,  2048, "torch.float16"),   # r=1.338 CI99-lo=1.330 p(<1.10)=0.0000
    (8192, 2048,  4096, "torch.bfloat16"),  # r=1.261 CI99-lo=1.256 p(<1.10)=0.0000
    (8192, 2048,  4096, "torch.float16"),   # r=1.227 CI99-lo=1.223 p(<1.10)=0.0000
    (8192, 2048,  8192, "torch.bfloat16"),  # r=1.202 CI99-lo=1.201 p(<1.10)=0.0000
    (8192, 2048, 16384, "torch.bfloat16"),  # r=1.192 CI99-lo=1.185 p(<1.10)=0.0000
    (8192, 2048, 16384, "torch.float16"),   # r=1.142 CI99-lo=1.132 p(<1.10)=0.0000
    (8192, 2048, 32768, "torch.float16"),   # r=1.117 CI99-lo=1.105 p(<1.10)=0.0002
})
assert len(_K1438_P17_SKINNY_N2048_KCOMPL_ROUTEOUT_N) == 20, (
    "_K1438_P17_SKINNY_N2048_KCOMPL_ROUTEOUT_N must be exactly 20 cells "
    "(K-1438 envelope sweep over M ∈ {2048,4096,8192} × N=2048 × "
    "K ∈ {2048,4096,8192,16384,32768} × {bf16,fp16} = 30 candidates; "
    "21 pass the strict K-1442 admit gate (ratio_median ≥ 1.10 ∧ bootstrap "
    "p(<1.10) < 0.01) but the 21st admit cell (2048,2048,2048,fp16) is "
    "DEFERRED to K-1295 P12 square_mid (already routes the cell at the "
    "6th position, ahead of this 11th-position predicate) — observable "
    "routing behaviour for the deferred cell is unchanged (still routes "
    "to hipBLASLt) but admitting it into both frozensets would violate "
    "the K-1175 / R-1329 pairwise-disjointness invariant.  See the "
    "_k1442_p17_skinny_n2048_routeout docstring for the full PRD-vs-"
    "frozenset cardinality rationale.)")
# Cross-frozenset disjointness — K-1442 P17 vs prior 10-predicate stack.
_K1442_P17_VS_P8_DISJOINT = (
    _K1438_P17_SKINNY_N2048_KCOMPL_ROUTEOUT_N.isdisjoint(_P8_MFMA_ISSUE_STALL_ROUTEOUT))
assert _K1442_P17_VS_P8_DISJOINT, (
    "K-1442 P17 skinny_N2048 K-COMPLEMENT cell overlaps the K-1322 51-cell "
    "P8 envelope; P8's K-1205/K-1219 N-axis sub-frozensets use N ∈ {128, 256}, "
    "K-1121/K-1131/K-1283 anchors are square M=N ∈ {1024, 2048, 4096}; "
    "P17's only square M=N=2048 cell is (2048,2048,2048,fp16) which is fp16 "
    "(P8 anchors are bf16 at this key) — natural disjointness, asserted as "
    "cheap insurance per R-1329 K-AXIS-PROJECTION-DISJOINTNESS-ASSERTS-ARE-"
    "CHEAP-INSURANCE.")
_K1442_P17_VS_K971_DISJOINT = (
    _K1438_P17_SKINNY_N2048_KCOMPL_ROUTEOUT_N.isdisjoint(K971_ROUTE_TABLE))
assert _K1442_P17_VS_K971_DISJOINT, (
    "K-1442 P17 skinny_N2048 K-COMPLEMENT cell overlaps K971_ROUTE_TABLE; "
    "K971_ROUTE_TABLE (K-905/K-971 + K-1335) uses M=N ∈ {1024, 2048} square "
    "with K ∈ {16384, 32768}; P17's only M=N=2048 cell is "
    "(2048,2048,2048,fp16) at K=2048 (no overlap with K971_ROUTE_TABLE's "
    "K ∈ {16384, 32768}) — natural disjointness, asserted insurance.")
_K1442_P17_VS_P12_DISJOINT = (
    _K1438_P17_SKINNY_N2048_KCOMPL_ROUTEOUT_N.isdisjoint(_K1295_P12_PMC_SQUARE_MID_ROUTEOUT_4))
assert _K1442_P17_VS_P12_DISJOINT, (
    "K-1442 P17 skinny_N2048 K-COMPLEMENT cell overlaps K-1361 P12 "
    "square_mid; P12 cells are M=N=K ∈ {2048, 4096} bf16+fp16 (4 cells); "
    "K-1442 explicitly drops (2048,2048,2048,fp16) from the P17 frozenset "
    "because P12 already routes that key at the 6th position; if this assert "
    "fires, a P12-overlapping cell has been re-introduced into the P17 set "
    "(undo the offending edit per K-1175 / R-1329 disjointness invariant).")
_K1442_P17_VS_K1367_P13_DISJOINT = (
    _K1438_P17_SKINNY_N2048_KCOMPL_ROUTEOUT_N.isdisjoint(
        _K1367_P13_SKINNY_N128_KCOMPL_ROUTEOUT_18))
assert _K1442_P17_VS_K1367_P13_DISJOINT, (
    "K-1442 P17 skinny_N2048 cell overlaps K-1367 P13 skinny_N128; "
    "P13(N=128) cells use N=128, P17 cells use N=2048 — natural "
    "disjointness, asserted for completeness (A4 sibling-N firewall).")
_K1442_P17_VS_K1397_P13_DISJOINT = (
    _K1438_P17_SKINNY_N2048_KCOMPL_ROUTEOUT_N.isdisjoint(
        _K1397_P13_SKINNY_N256_KCOMPL_ROUTEOUT_12))
assert _K1442_P17_VS_K1397_P13_DISJOINT, (
    "K-1442 P17 skinny_N2048 cell overlaps K-1397 P13 skinny_N256; "
    "P13(N=256) cells use N=256, P17 cells use N=2048 — natural "
    "disjointness, asserted for completeness (A4 sibling-N firewall).")
_K1442_P17_VS_K1409_P15_DISJOINT = (
    _K1438_P17_SKINNY_N2048_KCOMPL_ROUTEOUT_N.isdisjoint(
        _K1409_P15_SKINNY_N512_KCOMPL_ROUTEOUT))
assert _K1442_P17_VS_K1409_P15_DISJOINT, (
    "K-1442 P17 skinny_N2048 cell overlaps K-1417 P15 skinny_N512 "
    "EXTENSION; P15 cells use N=512, P17 cells use N=2048 — natural "
    "disjointness, asserted for completeness (A4 sibling-N firewall).")
_K1442_P17_VS_K1433_P16_DISJOINT = (
    _K1438_P17_SKINNY_N2048_KCOMPL_ROUTEOUT_N.isdisjoint(
        _K1433_P16_SKINNY_N1024_KCOMPL_ROUTEOUT))
assert _K1442_P17_VS_K1433_P16_DISJOINT, (
    "K-1442 P17 skinny_N2048 cell overlaps K-1433 P16 skinny_N1024 BASE; "
    "P16 cells use N=1024, P17 cells use N=2048 — natural disjointness, "
    "asserted for completeness (A4 sibling-N firewall; the five "
    "K-COMPLEMENT predicates partition the skinny-N column-narrow regime "
    "by N-axis at {128, 256, 512, 1024, 2048}).")

# ---------------------------------------------------------------------------
# K-1480 P19 (12th-position): skinny_N8192 K-COMPLEMENT 11-cell strict-equality
# route-OUT — extends the K-1389 → K-1400 → K-1409 → K-1433 → K-1442 → K-1458
# K-COMPLEMENT lineage one bucket up the N axis to N=8192.
#
# K-1480 supersedes the K-1465 staging frozenset (29 cells under the prior
# strict ≥1.10 gate measured on an OCI MI300X host) and the K-1468 staging
# frozenset (7 cells under ≥1.10 measured on an MI300X fallback host).
# After the K-1465↔K-1468 cluster/baseline divergence (R-1468 #5) the
# K-1480 brief mandated a strict-equality admit gate at the originally-
# specified ≥1.05x level on the K-1468 paired n=30 HIP-graph re-pairing:
#       ratio_median   ≥ 1.05  AND
#       ratio_ci99_lo  ≥ 1.05
# 11/30 cells pass; admit-set cohort geomean tb/hbl = 1.174× (range
# 1.056×–1.410×); 19 reject (12 bf16 + 7 fp16, including 4 cells in the
# 1.04–1.10 wedge that fail the strict CI gate).  Cohort sits strictly on
# the N=8192 column so disjointness with the 11 prior frozensets is by
# construction (R-1329 K-AXIS-PROJECTION-DISJOINTNESS-ASSERTS-ARE-CHEAP-
# INSURANCE — asserted explicitly below for every prior P0..P18 head).
#
# Source measurement: K-1468 paired n=30 HIP-graph hot-cache benchmarks on
# MI300X / gfx942 (an MI300X fallback host) with
# B=10000 vectorised paired bootstrap on per-iteration ratio array (R-1298 /
# R-1367).  Cells are the 30-cell N=8192 sweep
# M ∈ {2048, 4096, 8192} × N=8192 × K ∈ {2048, 4096, 8192, 16384, 32768}
# × dtype ∈ {torch.bfloat16, torch.float16}.
#
# Per-cell speedup deltas vs K-1458 P18 head (pre-P19 baseline) are
# documented in `output/p19_n8192_gist.md`.  Envelope grows by +11 cells
# (K-1458 head 28 cells → K-1480 head 28 + 11 = 39 cells on the K-COMPLEMENT
# stack, with no overlap into the K-1442 N=2048 column or the K-1458 N=4096
# column).
_K1465_P19_SKINNY_N8192_KCOMPL_ROUTEOUT = frozenset({
    # M=2048 row × N=8192 × K × dtype (6/10 admit at ≥1.05x strict-CI gate)
    ...
    # M=4096 row × N=8192 × K × dtype (4/10 admit; fp16-only)
    ...
    # M=8192 row × N=8192 × K × dtype (1/10 admit; saturated cohort —
    # tritonblas is competitive or ahead at every other M=8192 cell, so
    # only the M=8192 K=2048 fp16 cell makes it through the gate)
    (8192, 8192,  2048, "torch.float16"),    # r=1.098 CI99-lo=1.091
})
assert len(_K1465_P19_SKINNY_N8192_KCOMPL_ROUTEOUT) == 11, (
    "_K1465_P19_SKINNY_N8192_KCOMPL_ROUTEOUT must be exactly 11 cells "
    "(K-1480 P19 strict-equality admit gate at ≥1.05x on the K-1465/K-1468 "
    "30-cell N=8192 K-COMPLEMENT sweep: ratio_median ≥ 1.05 ∧ "
    "ratio_ci99_lo ≥ 1.05).  Cohort geomean tb/hbl = 1.174× over the 11 "
    "admit cells; range 1.056×–1.410×.  19 sweep cells fail the gate, "
    "including 4 cells in the 1.04–1.10 wedge whose CI99-lo dips below "
    "1.05 — those route through the native triton dispatch instead of "
    "hipBLASLt.  K-1480 supersedes the K-1465 29-cell staging set (≥1.10 "
    "gate) and the K-1468 7-cell staging set (≥1.10 gate, smaller cohort) "
    "after the R-1468 #5 cluster/baseline divergence reconciliation.")
...
_K1465_P19_VS_P8_DISJOINT = (
    _K1465_P19_SKINNY_N8192_KCOMPL_ROUTEOUT.isdisjoint(_P8_MFMA_ISSUE_STALL_ROUTEOUT))
assert _K1465_P19_VS_P8_DISJOINT, (
    "K-1465 P19 skinny_N8192 K-COMPLEMENT cell overlaps the K-1322 51-cell "
    "P8 envelope; P8 sub-frozensets use N ∈ {128, 256, 1024, 2048} for the "
    "K-1121/K-1131/K-1205/K-1219 anchors and N ∈ {2560, 3072, 3584, 4480} "
    "for the K-1283 A1 perturbations — none ever N=8192; natural disjointness, "
    "asserted as cheap insurance per R-1329.")
_K1465_P19_VS_K971_DISJOINT = (
    _K1465_P19_SKINNY_N8192_KCOMPL_ROUTEOUT.isdisjoint(K971_ROUTE_TABLE))
assert _K1465_P19_VS_K971_DISJOINT, (
    "K-1465 P19 skinny_N8192 K-COMPLEMENT cell overlaps K971_ROUTE_TABLE; "
    "K971_ROUTE_TABLE (K-905/K-971 + K-1335) uses M=N ∈ {1024, 2048} square "
    "with K ∈ {16384, 32768}; no N=8192 entries — natural disjointness, "
    "asserted insurance.")
_K1465_P19_VS_P12_DISJOINT = (
    _K1465_P19_SKINNY_N8192_KCOMPL_ROUTEOUT.isdisjoint(_K1295_P12_PMC_SQUARE_MID_ROUTEOUT_4))
assert _K1465_P19_VS_P12_DISJOINT, (
    "K-1465 P19 skinny_N8192 K-COMPLEMENT cell overlaps K-1361 P12 "
    "square_mid; P12 cells are M=N=K ∈ {2048, 4096} bf16+fp16 (4 cells) — "
    "no N=8192 entries; natural disjointness, asserted as cheap insurance "
    "per R-1329.")
_K1465_P19_VS_K1367_P13_DISJOINT = (
    _K1465_P19_SKINNY_N8192_KCOMPL_ROUTEOUT.isdisjoint(
        _K1367_P13_SKINNY_N128_KCOMPL_ROUTEOUT_18))
assert _K1465_P19_VS_K1367_P13_DISJOINT, (
    "K-1465 P19 skinny_N8192 cell overlaps K-1367 P13 skinny_N128; "
    "P13(N=128) cells use N=128, P19 cells use N=8192 — natural "
    "disjointness, asserted for completeness (A4 sibling-N firewall).")
_K1465_P19_VS_K1397_P13_DISJOINT = (
    _K1465_P19_SKINNY_N8192_KCOMPL_ROUTEOUT.isdisjoint(
        _K1397_P13_SKINNY_N256_KCOMPL_ROUTEOUT_12))
assert _K1465_P19_VS_K1397_P13_DISJOINT, (
    "K-1465 P19 skinny_N8192 cell overlaps K-1397 P13 skinny_N256; "
    "P13(N=256) cells use N=256, P19 cells use N=8192 — natural "
    "disjointness, asserted for completeness (A4 sibling-N firewall).")
_K1465_P19_VS_K1409_P15_DISJOINT = (
    _K1465_P19_SKINNY_N8192_KCOMPL_ROUTEOUT.isdisjoint(
        _K1409_P15_SKINNY_N512_KCOMPL_ROUTEOUT))
assert _K1465_P19_VS_K1409_P15_DISJOINT, (
    "K-1465 P19 skinny_N8192 cell overlaps K-1417 P15 skinny_N512 "
    "EXTENSION; P15 cells use N=512, P19 cells use N=8192 — natural "
    "disjointness, asserted for completeness (A4 sibling-N firewall).")
_K1465_P19_VS_K1433_P16_DISJOINT = (
    _K1465_P19_SKINNY_N8192_KCOMPL_ROUTEOUT.isdisjoint(
        _K1433_P16_SKINNY_N1024_KCOMPL_ROUTEOUT))
assert _K1465_P19_VS_K1433_P16_DISJOINT, (
    "K-1465 P19 skinny_N8192 cell overlaps K-1433 P16 skinny_N1024 BASE; "
    "P16 cells use N=1024, P19 cells use N=8192 — natural disjointness, "
    "asserted for completeness (A4 sibling-N firewall; the six "
    "K-COMPLEMENT predicates partition the skinny-N column-narrow regime "
    "by N-axis at {128, 256, 512, 1024, 2048, 8192} — N=4096 reserved for "
    "K-1458 P18, currently un-shipped).")
_K1465_P19_VS_K1442_P17_DISJOINT = (
    _K1465_P19_SKINNY_N8192_KCOMPL_ROUTEOUT.isdisjoint(
        _K1438_P17_SKINNY_N2048_KCOMPL_ROUTEOUT_N))
assert _K1465_P19_VS_K1442_P17_DISJOINT, (
    "K-1465 P19 skinny_N8192 cell overlaps K-1442 P17 skinny_N2048; "
    "P17 cells use N=2048, P19 cells use N=8192 — natural disjointness, "
    "asserted for completeness (A4 sibling-N firewall).")


def _k1465_p19_skinny_n8192_routeout(
    M: int, N: int, K: int, dtype, _dtype_str: str | None = None
) -> bool:
    """K-1480 P19 — direct hipBLASLt route-OUT for the 11-cell skinny_N8192
    K-COMPLEMENT envelope (`_K1465_P19_SKINNY_N8192_KCOMPL_ROUTEOUT`).

    Returns True iff (M, N, K, dtype) matches one of the 11 strict-equality
    keys productionised from the K-1480 N=8192 K-COMPLEMENT 30-cell sweep
    under the originally-specified ≥1.05x strict admit gate
    (ratio_median ≥ 1.05 ∧ ratio_ci99_lo ≥ 1.05).  The cohort sits strictly
    on the N=8192 column:
      M ∈ {2048, 4096, 8192} × N=8192 × K ∈ {2048, 4096, 8192, 16384, 32768}
      × dtype ∈ {torch.bfloat16, torch.float16}.  10 of 11 admit cells are
      fp16; the lone bf16 admit is (2048, 8192, 8192, bf16) at r=1.102 — the
      bf16 axis otherwise inverts at N=8192 (12/15 bf16 cells have r<1.0,
      tritonblas wins; min r=0.74 at (4096, 8192, 16384, bf16)).  M=8192
      saturates almost entirely (1/10 admit, only K=2048 fp16); M=2048 and
      M=4096 contribute the bulk of the lift.

    Cohort geomean tb/hbl = 1.174× over the 11 productionised cells (range
    1.056×–1.410×); min CI99-lo = 1.055 at (4096, 8192, 16384, fp16); max
    ratio_median = 1.410× at (2048, 8192, 8192, fp16).

    K-1480 supersedes both staging frozensets:
      * K-1465 P19 (29 cells, ≥1.10 gate, an OCI MI300X host)
      * K-1468 P19 (7 cells, ≥1.10 gate, an MI300X fallback host)
    after the R-1468 #5 cluster/baseline divergence reconciliation.  The
    K-1480 ≥1.05x gate applied to the K-1468 paired n=30 measurements
    on the MI300X fallback host is the canonical productionisation envelope.
    ...
    cross-frozenset asserts above.  Hot-path note: same `_dtype_str` kwarg
    convention as K-1442 P17 (R-1442 #4 torch-free-predicate-dispatch
    invariant; pre-stringified at call-site by `k971_route_decision`).
    """
    if _dtype_str is None:
        _dtype_str = str(dtype)
    return (
        (int(M), int(N), int(K), _dtype_str)
        in _K1465_P19_SKINNY_N8192_KCOMPL_ROUTEOUT
    )


def _k1442_p17_skinny_n2048_routeout(
    M: int, N: int, K: int, dtype, _dtype_str: str | None = None
) -> bool:
    """K-1442 P17 — direct hipBLASLt route-OUT for the 20-cell skinny_N2048
    K-COMPLEMENT envelope (`_K1438_P17_SKINNY_N2048_KCOMPL_ROUTEOUT_N`).

    Returns True iff (M, N, K, dtype) matches one of the 20 strict-equality
    keys productionised from the K-1438 30-cell envelope sweep at N=2048:
      M ∈ {2048, 4096, 8192} × N=2048 × K ∈ {2048, 4096, 8192, 16384, 32768}
      × dtype ∈ {torch.bfloat16, torch.float16}.

    PRD-vs-frozenset cardinality (admit gate vs productionised set):
      The PRD specifies ``emit cells where hipBLASLt wins by ≥10% with
      p<0.01``.  The K-1442 paired n=30 / B=10000 vectorised paired
      bootstrap admit gate (ratio_median ≥ 1.10 ∧ bootstrap p(<1.10) < 0.01)
      passed **21/30** sweep cells.  Of those, **20** are productionised
      into ``_K1438_P17_SKINNY_N2048_KCOMPL_ROUTEOUT_N``; the 21st admit
      cell is **deferred** rather than dropped:

        * (2048, 2048, 2048, fp16) — r=1.121, CI99-lo=1.100, p(<1.10)=0.0049

      This cell is already routed by ``_K1295_P12_PMC_SQUARE_MID_ROUTEOUT_4``
      (M=N=K ∈ {2048, 4096} bf16+fp16) at the 6th-position dispatch in
      ``k971_route_decision``, ahead of this 11th-position predicate.  Per
      K-1175 / R-1329 (``K-AXIS-PROJECTION-DISJOINTNESS-ASSERTS-ARE-CHEAP-
      INSURANCE``) the predicate stack is required to be pairwise-disjoint
      so cells route through exactly one path; admitting this cell into the
      P17 frozenset would violate the invariant and trip
      ``_K1442_P17_VS_P12_DISJOINT`` at module load.  The cell is therefore
      omitted from the P17 frozenset (cardinality 20, not 21) but its
      observable routing behaviour is preserved — every cell where the K-1442
      admit gate fires routes to hipBLASLt, just via P12 instead of P17 for
      the deferred cell.  This is a defensible engineering trade per
      R-1329 (disjointness invariant > strict-cardinality match against PRD)
      and is documented in ``_K1438_P17_SKINNY_N2048_KCOMPL_ROUTEOUT_N``'s
      own length-assert message.

    Source measurement: K-1442 paired n=30 HIP-graph hot-cache benchmarks
    on MI300X / gfx942 (OCI MI300X fallback an OCI fallback host; c42 down)
    with B=10000 vectorised paired bootstrap on per-iteration ratio array
    (R-1298 / R-1367).  Cohort geomean tb/hbl = 1.335× over the 20
    productionised cells (range 1.142×–1.690×); min CI99-lo = 1.105 at
    canary (8192, 2048, 32768, fp16); max ratio_median = 1.690× at
    (4096, 2048, 4096, bf16).

    Mechanism: at N=2048 the column-narrow LDS layout still triggers the
    K-913 §3 LDS-bank-conflict signature on M ∈ {4096, 8192} rows
    (10/10 + 8/10 admit), but attenuates on the small-M (M=2048) row
    where TB persistent_matmul becomes competitive (3/10 admit, only the
    fp16 small-K subset). hipBLASLt's split-K kernel selection sidesteps
    the LDS conflict on the K-mid band (K ∈ {4096, 8192}) and switches
    to a different kernel family at the K-edges (K ∈ {16384, 32768})
    that does not penalise the wider tile.

    N-axis trajectory across the K-COMPLEMENT lineage (cohort geomean):
        K-1389 N=128  cohort geomean = 1.678×
        K-1400 N=256  cohort geomean = 1.471×
        K-1409 N=512  cohort geomean = 1.372×
        K-1433 N=1024 cohort geomean = 1.451× (R-1409 monotone reversed)
        K-1442 N=2048 cohort geomean = 1.335× (mild re-attenuation; the
            K-1131 A2 1.40× cohort floor is MISSED at N=2048 by 6.5 pp,
            mirroring the K-1409 N=512 pattern — supports R-1442 #1
            "K-COMPLEMENT discriminator non-monotone in N with damped
            oscillation around the 1.40× floor").
    ...
    Hot-path note (K-1442 R-Performance-Hawk follow-up): the frozenset is
    keyed on the *string* ``str(dtype)`` (``"torch.bfloat16"`` /
    ``"torch.float16"``) rather than the ``torch.dtype`` singleton, because
    this module is **torch-free by design** (see file docstring lines 1–12
    — ``importable on its own (no torch / triton chain)``) and the test
    harness passes string-literal dtypes for unit/integration tests that
    must exercise this predicate without booting a GPU stack.  Importing
    torch here to obtain the dtype singletons would break both invariants
    AND diverge from the 9 prior frozensets in this file.  To avoid the
    ``str(dtype)`` allocation on every dispatch (raised by R-Performance-
    Hawk on K-1442's first revision), ``k971_route_decision`` now
    pre-stringifies ``a_dtype`` once at the top of the function and passes
    the cached string into this predicate via the ``_dtype_str`` keyword
    argument; when ``_dtype_str`` is supplied the predicate skips the
    ``str(dtype)`` call entirely and the 11th-position membership test
    reduces to a single ``frozenset.__contains__`` (O(1) hash + interned-
    string pointer-compare; zero per-call allocation).  The ``_dtype_str``
    argument keeps the public ``(M, N, K, dtype)`` signature back-compatible
    ...
    return (
        (int(M), int(N), int(K), _dtype_str)
        in _K1438_P17_SKINNY_N2048_KCOMPL_ROUTEOUT_N
    )
...
def k971_route_decision(M, N, K, a_dtype, b_dtype, enable_streamk,
    ...
    Precedence (top-down, first match wins):
     ...
     11. K-1442 P17 skinny_N2048 K-COMPLEMENT 20-cell strict-equality -> hipBLASLt.
     12. K-1480 P19 skinny_N8192 K-COMPLEMENT 11-cell strict-equality -> hipBLASLt.
    """
    if disable_env_set:
        return False
    ...
    if _k1433_p16_skinny_n1024_routeout(int(M), int(N), int(K), a_dtype):
        return True
    # K-1442 P17 (11th-position): skinny_N2048 K-COMPLEMENT 20-cell route-OUT.
    # Stacks AFTER K-1433 P16 per K-1175 stacked-predicate convention; closes
    # the next-higher N tier (N=2048) along the K-1389 → K-1400 → K-1409 →
    # K-1433 K-COMPLEMENT lineage.  Per-cell ratios 1.142×–1.690×; cohort
    # geomean 1.335×; envelope grows 115 → 135 cells.  At N=2048 the M=2048
    # row collapses on most cells (TB persistent_matmul becomes competitive
    # at the small-M / wide-N column quadrant) — only 2/10 small-M cells admit
    # here (a third small-M cell, (2048,2048,2048,fp16), is already routed
    # by K-1295 P12 square_mid at the 6th position); the M ∈ {4096, 8192}
    # rows remain LDS-bank-conflict dominated (10/10 + 8/10 admit respectively).
    # Two sub-cohorts also reject: (8192,2048,8192,fp16) r=1.033 and
    # (8192,2048,32768,bf16) r=1.004 — both fall through to native triton.
    #
    # K-1442 R-Performance-Hawk follow-up: the predicate is a pure frozenset
    # membership test keyed on (M, N, K, str(dtype)) — to keep the 11th-
    # position dispatch a zero-allocation pointer-compare we pre-stringify
    # a_dtype ONCE here and pass the cached string into the predicate via
    # the _dtype_str kwarg.  This module is torch-free by design (file
    # docstring lines 1-12) and the test harness passes string-literal
    # dtypes, so the frozenset cannot key on the torch.dtype singleton
    # directly without breaking both invariants — caching the string at
    # the call site is the right place to amortise the conversion.
    _a_dtype_str = str(a_dtype)
    if _k1442_p17_skinny_n2048_routeout(
        int(M), int(N), int(K), a_dtype, _dtype_str=_a_dtype_str
    ):
        return True
    # K-1480 P19 (12th-position): skinny_N8192 K-COMPLEMENT 11-cell route-OUT.
    # Stacks AFTER K-1442 P17 per K-1175 stacked-predicate convention; closes
    # the next-higher-than-N=4096 tier of the K-1389 → K-1400 → K-1409 → K-1433
    # → K-1442 K-COMPLEMENT lineage at N=8192 under the originally-specified
    # ≥1.05x strict-CI admit gate (ratio_median ≥ 1.05 ∧ ratio_ci99_lo ≥ 1.05).
    # Per-cell ratios 1.056×–1.410×; cohort geomean 1.174×; envelope grows by
    # +11 cells.  11/30 ADMIT — 10 fp16 + 1 bf16; the bf16 axis inverts at
    # N=8192 (12/15 bf16 cells have r<1.0, tritonblas wins).  N=8192 axis is
    # trivially disjoint with the M=N=K∈{2048,4096} P12 envelope so no P12
    # deferral is required.  K-1480 supersedes the K-1465 (29-cell ≥1.10 OCI)
    # and K-1468 (7-cell ≥1.10 MI300X fallback) staging frozensets after the R-1468 #5
    # cluster/baseline divergence reconciliation.
    if _k1465_p19_skinny_n8192_routeout(
        int(M), int(N), int(K), a_dtype, _dtype_str=_a_dtype_str
    ):
        return True
    return False
