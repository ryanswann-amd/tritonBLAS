...
# E_N3 N-axis extension admits.  Paired n=30 HIP-graph hot-cache validation
# on rad-mi300x-1 (MI300X gfx942; documented MI300X fallback per the
# anchor-to-neighbor / E2 / E1 measurement campaigns) of the N-floor
# relaxation envelope:
#   E_N3 := N = 128 (one power-of-2 tier below the natural sub-1024 N floor)
#           AND K in {256, 768, 1024} (held fixed at the source K-set)
#           AND M in the established anchor M-set
#           AND bf16
# returned a clean POSITIVE landing verdict: 8/9 = 88.9% admit, well above
# the 75% landing threshold and *decisively diverging* from the prior
# K-axis NEGATIVE_AXIS_PIVOT result (1/6 = 16.7% K-axis admit; cross-arch
# audit confirmed arch-agnostic).
#
...
# bottleneck that the source anchors expose at N=2048; tritonblas's wpeu=1
# in-kernel admit is *worse* at small-N skinny-rectangle tiles than at the
# original mid-square anchor tiles (per-cell speedups widen 1.14x-3.25x at
...
# small-M Triton-favoured tail surfaced by the K-axis audit is active here
# at the M-floor of the anchor M-set.
#
...
# neighbors, E2 admits, E_N3 N-axis admits) -- the dispatch path consults
# the union.  Per the dual-frozenset provenance rule and its E2 extension,
# source-campaign lineage is load-bearing for future reviewers
# (precedence-inversion debugging, PMC re-classifier work, ADR audits) so
# each measurement campaign keeps its own named set with a runtime size
# + disjointness check.
...
assert len(_P8_MFMA_ISSUE_STALL_ROUTEOUT) == 36, (
    ...
    "3 E2 admits + 8 E_N3 N-axis admits); a duplicate or stray entry "
    "has crept in.")
# Cross-check: the four sub-sets must be pairwise disjoint by construction.
# Neighbors perturbed AWAY from anchors; E2 admits selected from the
# always-uncovered top-40 catalog minus all anchors and minus all
# neighbors; E_N3 admits use N=128 (which is in NO prior P8 cell -- prior
# P8 cells all have N >= 896).
...
assert _K1131_P8_NEIGHBORS_12.isdisjoint(_K1161_E2_ADMITS_3), (
    "E2 admits overlap with neighbors; the E2 candidate generator excluded "
    "all neighbors by construction.")
...
def _p8_mfma_issue_stall_routeout(M: int, N: int, K: int, dtype) -> bool:
    """P8 (extended through E_N3) — direct hipBLASLt route-OUT for the
    quadruply-validated MFMA-issue-stall cohort (anchors + neighbors +
    E2 K-interior/M-axis admits + E_N3 N-axis admits at N=128).
