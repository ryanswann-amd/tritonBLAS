...
from ._route_predicate import (
    ...
    # 18-cell envelope (M ∈ {2048,4096,8192} × N=1008 ×
    # K ∈ {4096,8192,16384} × {bf16,fp16}).  Off-by-48 wave-misaligned
    # 4th rung (1008 mod 64 = 48); K-2031 paired n=30 HIP-graph hot-cache
    # MI300X / gfx942 vs live post-K-1922 oracle (HEAD 0024a71): BEFORE
    # cohort geomean tb/hbl = 1.326× (range 0.995×–1.631×, 16/18 ≥1.10×
    # admit, 16/18 ≥1.05× admit).  Mirrors the K-1978 P45 N=816 /
...
def _k971_route_to_hbl(M, N, K, a_dtype, b_dtype, enable_streamk, work_stealing):
    ...
    # K-2031 (S-002): P47 (37th-slot) skinny_N1008 K-COMPLEMENT alias-stack — 18 cells
    # (off-by-48 wave-misaligned 4th-rung; BEFORE cohort geomean tb/hbl = 1.326×,
