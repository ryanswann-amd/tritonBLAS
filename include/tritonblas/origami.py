...
class OrigamiMatmulSelector:
    ...
    def __init__(
        ...
        #
        #  1. Intra-workgroup thread occupancy.  The workgroup of 512 threads
        #     processes BLOCK_M * BLOCK_N output elements per K iteration.
        #     With BLOCK_M=16 BLOCK_N=32 that is 512 elements -- one element
        #     per thread, leaving most of the MFMA pipeline idle.  Raising
        #     BLOCK_N to 256 lifts per-thread work to >= 8 elements.
        #
        #  2. Per-launch arithmetic intensity.  BLOCK_K=32 doubles the K-loop
        #     iteration count vs. BLOCK_K=64 and halves the LDS-load-to-MFMA
        #     ratio inside the kernel body.
        #
        # The override picks (BLOCK_M, BLOCK_N, BLOCK_K) = (16|32, 256, 64).
        # CU coverage is recovered by enabling the Stream-K code path on the
        # call site (matmul auto-enables enable_streamk for M<=32) which
        # multiplies the launch grid by the SPLIT_K factor (typically 2-8x).
        #
