"""C07_BK128_kp2 21st-slot envelope frozenset — pin tests.

8-cell M=N=2048 K-COMPL mid-K cohort: cardinality (8), structure
(M=N=2048 × K∈{2048,4096,8192,16384} × {bf16,fp16}), disjoint vs sibling-N
N=64/N=128 prior route-OUT slots, intentional alias-subset of the P26
N=2048 17th-slot (which routes these 8 cells to hipBLASLt FIRST in the
production dispatch chain), and explicit exclusion of adjacency cells
(M=N=2048 K∈{1024,32768} and M=N∈{1536,3072} K=4096).  The frozenset is
a documentation/regression-safety data marker — no inline tile-config
override is wired into persistent_matmul_lt, since C07 measured strictly
worse than hipBLASLt and would regress the cohort if it superseded P26.
"""
import pytest
