...
def persistent_matmul_lt(
    ...
    # K-751 / K-736: FP8 tall-skinny tile override (composite-13 NEW).
    # Routed FIRST among all FP8 hooks (DISPATCH-ORDER-FIRST) so that an
    # in-cohort (M,N,K)+e4m3fnuz triple cannot be re-routed by a sibling
    # FP8 hook (K-695-style large-N override below). This forecloses the
    # K-654/K-683 cohort-leakage bug class by construction, not by audit.
    # K-751 cohort is FP8 e4m3fnuz tall-skinny (M >= 4096 / N <= 128) —
    # disjoint by dtype from K-693 (FP16/BF16), disjoint by M-shape from
    # K-695 (M in {16,32,64} small-M MIRROR cohort). No overlap with any
    # K-730 / K-752 composite-12 gate.
    ...
    # the medium-K residual band. K-683 predicate is FP16/BF16-only — does
    # NOT touch K-751's FP8 e4m3fnuz cohort.
    ...
    # K-695: tile override for FP8 e4m3fnuz tall-skinny cohort (small-M,
    # large-N MIRROR of K-751). LDS-fit guard falls back to Origami on the
    # (rare) BN/BK combo where the override exceeds the 64KiB MI300X
    # workgroup LDS budget. Applied AFTER K-683 (FP16/BF16-only, no
    # overlap) AND AFTER K-751 (FP8 strict-equality first-routing). The
    # explicit `_fp8_ts_ov is None` guard makes the dispatch-order-first
    # invariant load-bearing rather than depending only on shape disjoint-
    # ness — defensive against future K-695 cohort widening.
    _ovr = _k695_tile_override(M, N, K, a.dtype)
    if _ovr is not None and not work_stealing and _fp8_ts_ov is None:
        _bm, _ns, _kp = _ovr
    ...
    # M=4096,N=64,K in {4096,8192} overlap region (K-693 tile geometry
    # needs the larger warp count for issue density). K-693 is FP16/BF16
    # only — cannot route to K-751's FP8 e4m3fnuz cohort.
