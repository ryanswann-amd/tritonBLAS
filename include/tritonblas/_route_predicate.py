...
# Skinny-N N=1840 K-COMPLEMENT alias-stack — 18-cell verified-winner
# cohort for the off-by-48 wave-misaligned N=1840.  16th-rung extension
# of the contiguous off-by-48 ladder (N=544, 816, 880, 944, 1008, 1072,
# 1136, 1200, 1264, 1328, 1392, 1456, 1584, 1648, 1712, 1776).
#
...
# mod-128=48 sub-class shared with N=1456 and N=1648 (alternation
# 48/112/48/112/... across the residue-48 lattice).  The K-COMPLEMENT
# swap recovers wave-tile coverage at residue-48 by reorienting the
# inner-K reduction so the per-tile schedule no longer strands a 48-
# element wave-misaligned N-fragment per BLOCK_N=128 tile (N=1840 =
# 14.375 fractional tiles per N-row).  Only `mod 64 == 48` binds — the
# `mod 128 == 48` subclass admits identically (validated upstream at
# N=1456 and N=1648 — both `mod 128 == 48`).
#
...
# Verified by paired n=30 HIP-graph hot-cache 3-engine bench (tb / hbl /
# rocblas) on the 18-cell envelope (M ∈ {2048,4096,8192} × N=1840 ×
# K ∈ {4096,8192,16384} × {bf16,fp16}); cohort geomean route-OUT/HBL
# = 1.015× (HBL parity), cohort uplift native/route-OUT = 1.187×.
#
# Naturally disjoint with every prior K-COMPLEMENT alias-stack slot (no
# shared N).  +2 active LOC additive in the dispatcher (1 import in
# matmul.py + 1 membership-check); cold-path latency impact bounded by a
...
# prior off-by-48 rungs already in the alias-stack.
_SKINNY_N1840_RESIDUE48_HBL_ROUTE = frozenset(
    (M, 1840, K, dt)
    for M in (2048, 4096, 8192)
    for K in (4096, 8192, 16384)
    for dt in ("torch.bfloat16", "torch.float16")
)
# Cardinality (==18) gated by tests/test_skinny_n1840_residue48_hbl_route.py
# per the minimalist split: src holds data, tests hold invariants
# (R-1532 / R-1720 / R-1775).
