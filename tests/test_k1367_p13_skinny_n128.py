...
`_K1367_P13_SKINNY_N128_KCOMP_ROUTEOUT_18` frozenset stacked as the
...
from tritonblas._route_predicate import (
    ...
    _K1367_P13_SKINNY_N128_KCOMP_ROUTEOUT_18,
    _P8_MFMA_ISSUE_STALL_ROUTEOUT,
    _k1361_p12_square_mid_routeout,
    _k1367_p13_skinny_n128_kcomp_routeout,
    k971_route_decision,
)
...
def test_envelope_total_post_p13_is_73():
    ...
    total = (
        ...
        + len(_K1367_P13_SKINNY_N128_KCOMP_ROUTEOUT_18)
    )
    assert total == 73, (
...
def test_p13_membership_exactly_18_kcomplement_cells():
    ...
    assert _K1367_P13_SKINNY_N128_KCOMP_ROUTEOUT_18 == expected, (
        "K-1367 P13 frozenset diverges from the K-COMPLEMENT bucket rule: "
        "M ∈ {2048,4096,8192} × N=128 × K ∈ {4096,8192,16384} × {bf16,fp16}.\n"
        f"  missing : {sorted(expected - _K1367_P13_SKINNY_N128_KCOMP_ROUTEOUT_18)}\n"
        f"  unknown : {sorted(_K1367_P13_SKINNY_N128_KCOMP_ROUTEOUT_18 - expected)}")
...
def test_p13_disjoint_from_p8():
    assert _K1367_P13_SKINNY_N128_KCOMP_ROUTEOUT_18.isdisjoint(
        _P8_MFMA_ISSUE_STALL_ROUTEOUT)
...
def test_p13_predicate_admits_every_kcomplement_cell(M, N, K, dtype):
    assert _k1367_p13_skinny_n128_kcomp_routeout(M, N, K, dtype) is True
...
def test_p13_predicate_rejects_axis_perturbations(M, N, K, dtype):
    assert _k1367_p13_skinny_n128_kcomp_routeout(M, N, K, dtype) is False
...
def test_p13_admit_cell_carveout_short_circuits(enable_streamk, work_stealing, b_dtype):
    ...
    M, N, K = 4096, 128, 8192
    assert (M, N, K, a_dtype) in _K1367_P13_SKINNY_N128_KCOMP_ROUTEOUT_18
