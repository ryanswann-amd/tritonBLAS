"""K-2111 P53 (S-002) — 10th-rung off-by-48 N=1392 K-COMPLEMENT alias-stack tests.
...
K-2085 (P50, P51) / K-2097 (P52).  K-2111 promotes N=1392 to the full
18-cell envelope M ∈ {2048, 4096, 8192} × N=1392 × K ∈ {4096, 8192,
16384} × {bf16, fp16} for production routing to hipBLASLt.

Off-by-48 ladder context (R-1811 wave-misalignment + K-913 §3 LDS-bank-conflict
mechanism, dtype-invariant): N=1392 is the 10th contiguous rung of the
off-by-48 wave-misaligned residue family above N ∈ {816, 880, 944, 1008,
1072, 1136, 1200, 1264, 1328} (K-1978 / K-1994 / K-2010 / K-2031 / K-2047 /
K-2055 / K-2067 / K-2085 P50 / K-2085 P51 / K-2097).  All ten rungs satisfy
(N mod 64) == 48.

Sub-family placement: N=1392 mod 128 == 112 — off-tail sub-family
shared with N ∈ {880, 1008, 1136, 1264} (vs the canonical-tail sub-family
at N ∈ {816, 944, 1072, 1200, 1328} where N mod 128 == 48).  Step +64
from the K-2097 P52 rung at N=1328.

Envelope choice: 18-cell K ∈ {4096, 8192, 16384} (matches the K-2055 P48
N=1136 + K-2097 P52 N=1328 18-cell envelope convention) — the K-2111 task
spec explicitly enumerates K ∈ {4096, 8192, 16384} for the 10th-rung
promotion sweep.
...
R-1720 / R-1775):
  ...
  6. Off-by-48 wave-misalignment band membership: 1392 mod 64 == 48 (same
     ladder as the 9 prior confirmed rungs).
  7. Off-tail sub-family membership: 1392 mod 128 == 112 (same sub-family
     as N ∈ {880, 1008, 1136, 1264}).
  8. Step +64 from the K-2097 P52 rung at N=1328 (contiguous-ladder
     extension invariant; guards against silent N-axis typos like 1382,
     1394, 1456 that would land on a different BLOCK_N=64 alignment rung).
"""
...
FZ = _K2111_P53_SKINNY_N1392_KCOMPL_ALIASSTACK_18
...
def test_dtype_row_balance():
    ...
    column-narrow N=1392 tile (same fingerprint as the 9 prior off-by-48
    rungs)."""
...
def test_envelope_equals_full_n1392_kcompl_grid():
    ...
    4096, 8192} × N=1392 × K ∈ {4096, 8192, 16384} × {bf16, fp16} grid
    ...
    leak non-winner cells out to HBL) of the K-2111 promotion envelope."""
    full = frozenset(
...
def test_off_by_48_wave_misalignment_ladder_membership():
    """N=1392 sits on the off-by-48 wave-misaligned ladder (N mod 64 == 48)
    above the 9 prior confirmed rungs N ∈ {816, 880, 944, 1008, 1072, 1136,
    1200, 1264, 1328}.  Step +64 from the K-2097 P52 N=1328 rung.  This
    invariant guards against a silent N-axis typo (e.g. 1382, 1394, 1456)
    that would land on a different BLOCK_N=64 alignment rung and reflect a
    different mechanism."""
    (only_n,) = {N for (_, N, _, _) in FZ}
    assert only_n % 64 == 48
    assert only_n == 1328 + 64
    assert only_n == 1392


def test_off_tail_sub_family_membership():
    """N=1392 mod 128 == 112 — off-tail sub-family shared with
    N ∈ {880, 1008, 1136, 1264} (R-1811 wave-misalignment sub-classification).
    Distinct from the canonical-tail sub-family at N mod 128 == 48
    (N ∈ {816, 944, 1072, 1200, 1328})."""
    (only_n,) = {N for (_, N, _, _) in FZ}
...
def test_sibling_n_firewall_vs_p40_n544():
    ...
    by natural N-axis separation (1392 ≠ 544); guards against a silent
    N-axis collision."""
